...
out over `1448`–`1460`. Per `STORYBOARD.md` §27 the metrics-to-logo path is
`fade → silence → brand reveal`, and scene 07 now hands that silence over
explicitly: its last rendered frame `1459` is one frame short of black, so the
...
- Global frames: `1460`–`1630`
- Scene-local frames: `0000`–`0170` (last rendered frame `0169` / master `1629`)
...
| Master        | Local         | Beat                                                     |
| ------------- | ------------- | -------------------------------------------------------- |
| `1460`        | `0000`        | Everything is gone. Black. Intentional silence.           |
| `1470`        | `0010`        | A single node appears.                                    |
| `1470`–`1510` | `0010`–`0050` | Thin lines arrive from outside the frame toward that node. |
| `1500`–`1530` | `0040`–`0070` | The relationships converge.                               |
| `1530`        | `0070`        | `kivgraph` reads.                                         |
| `1440`        | `0080`        | `Exact code intelligence for coding agents.` reads.        |
| `1440`–`1630` | `0080`–`0170` | Settled. Nothing moves.                                   |

## Initial state

At `1460` the frame is empty and dark: `background` `#0a0b0d`, edge to edge.
Nothing else. The ten frames from `1460` to `1470` are deliberately empty — this
...
At `1629` the frame holds the settled Kivgraph lockup and its tagline, centred
on `x = 960`:
...
- the convergence lines fully resolved — they dim out, and no fragment of them
  survives past `1524`, twenty-five frames before this;
- nothing in motion.
...
The single node at `1470` **is the mark itself**, at the centre line of the
...
brand blue on screen at all after `1530`. It carries an off-white `#e9e2dc` and
...
**Arrival (`1470`–`1510`).** Each line is drawn from outside the frame toward the
...
**Convergence (`1500`–`1530`).** The lines complete, meet at the node, and then
...
at `0056`, so `1516`–`1518` is the complete figure, measured byte-identical for
three frames. Measured, no line fragment survives past `1524` — four frames
...
**Naming (`1530` and `1440`).** The wordmark appears beneath the mark, then the
...
**The turn (`1520`–`1536`).** One full revolution of the mark, starting with the
...
poster. Ending at `1536` leaves the frame static from `1538`, which is where it
was static before the turn existed. It is a single turn that stops. It is never a
loop, and nothing turns after `1536` — scene 09 renders the lockup at `0°`.
...
`1440`, not begin there, and every line must be fully resolved before `1530`. If
anything is still moving at `1440`, the key frame is unusable.

Nothing moves between `1440` and `1630`. Measured, the frame is actually static
from `1538` — the entrance easing is within one 255th of its final value two
frames before its nominal end — so `1538`–`1629` is one image repeated ninety-two
times.
...
explain nothing the viewer has not already understood by `1460`.
...
`1630` into a DOM scene 09, and matching a projected 3D position to a DOM layout
...
first line is drawn at `1470`. The premount is for the scrubber, not for the
...
`07-benchmark.md` fades its four statements out together over `1448`–`1460`,
landing on an empty frame at `1460`. Its last rendered frame is `1459`, one frame
...
master `1460`–`1470`, before the node appears at `1470`.
...
mounts at `1460` and paints the brand background across them.
...
At `1630` the composition continues into `09-outro.md`.

**Derived continuity decision, not literal in the storyboard, and now built that
way.** The lockup stays on screen across the `1630` boundary, in the same
...
requires the name to still be there. A version where the lockup exits at `1630`
...
the `1290` still, with the same instruction: a still that needs optical centring
...
- there is no fade or cut at `1630` — the lockup simply does not move;
...
`kivgraph` column head of scene 07's table. The match cut at `1209`/`1210` hands
...
```text
frame 1460 — empty frame; the intentional silence
frame 1510 — relationships mid-arrival, converging on the mark
frame 1440 — complete lockup plus tagline, fully settled; STILL-IMAGE KEY FRAME
frame 1629 — unchanged from 1440; the frame the outro inherits, after ninety
...
`1460`, `1510`, and `1440` are all on the manual review list in
`STORYBOARD.md` §28.
...
- **The lockup is DOM.** Not in the R3F canvas, at any point, for any reason. Its
  screen position has to survive the `1630` boundary into a DOM scene 09, and a
  projected 3D position cannot guarantee that.
- The arriving lines are gone by the time the wordmark is on screen — measured,
  no fragment survives past `1524`. They never remain as part of the mark, and
...
- **The turn ends by `1536`.** Whatever its shape, the mark is motionless before
  `1538`. `1440` is a still-image key frame and a logo caught mid-rotation there
  is an unusable poster.
- **One turn, and it stops.** Never a loop, never a second revolution, and
  nothing turns after `1536` — scene 09 inherits the mark at `0°`.
- The mark is what the relationships converged on. Whatever else changes, the
  thing at the centre at `1470` and the thing in the lockup at `1629` are the
...
- Frame `1440` works as a still image: complete lockup, complete tagline, nothing
  mid-animation.
- The frame is static from `1440` to `1630`.
- The `1460`–`1470` emptiness is preserved.
...
- The exact tagline size within the 26–52 px range. **Built: 34 px**, the top of
  the body tier.
- The turn's duration and easing, **provided it concludes by `1536`**. **Built:
...
- Shared component, and it exists: `src/components/BrandLogo.tsx`, owning the
  ...
  owns the lockup's screen position, which is the shared constant the `1630`
...
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="08 Brand" from={1460} durationInFrames={170} premountFor={30}>`
  literals, because
...
- **The mark carries `will-change: transform`, for determinism and not for
  ...
  `1549` because that was the scene's last frame when it was measured; it is
  `1629` now): `remotion render
  --frames=1520-1449` and `--frames=1440-1449` disagreed at `1525`, `1530` and
  `1536`, while two renders of the same range were byte-identical to each other.
  ...
  verified after, `remotion still`, a 1440-start render and a 1520-start render
...
- **Override — the mark turns once.** This document's own `## Motion` says the
  ...
  override was bounded by the still frame: the turn concludes at `1536` and
  `1538`–`1629` is one static image. It was not free, though, and the cost is the
...
- **Override consequence — the film has no accent after `1530`.** `#2563eb` is
...
- **Sound absent.** `STORYBOARD.md` §18 asks for a *very soft impact* on the
  logo, around `1530`. Nothing is authored yet. Per §17 the sound may never be
...
- **The tagline's total dwell is now 210 frames, and that is worth a decision
  ...
  and inherits the lockup at `1630`, holding it to `1749`, so the line is on
...
- **R3F or 2D — resolved twice.** First in favour of 2D, on the argument that the
  ...
  across `1630`. Then overridden for the lines only, on direct art direction,
...
- **Lockup persistence across `1630` — resolved in favour of persistence.** See
