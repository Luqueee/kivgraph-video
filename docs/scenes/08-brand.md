...
- Global frames: `1360`–`1450`
- Scene-local frames: `0000`–`0090` (last rendered frame `0089` / master `1449`)
- Time: 22.67 s – 24.17 s
...
| `1440`–`1450` | `0080`–`0090` | Settled. Nothing moves.                                   |
...
At `1449` the frame holds the settled Kivgraph lockup and its tagline, centred:
...
The single node at `1370` should be the mark itself: a small solid square in
...
**Naming (`1430` and `1440`).** The wordmark appears beside or beneath the mark,
...
Nothing moves between `1440` and `1450`.
...
- **Camera intent:** effectively static. A very small dolly-in (well inside the
  ...
  perfectly still by `1430`, because the frame is a logo.
...
- **Screen-space parity:** if 3D is chosen, the mark's final screen position and
  ...
  across the `1450` boundary. Matching a projected 3D position to a DOM layout
...
At `1450` the composition continues into `09-outro.md`.

**Derived continuity decision, not literal in the storyboard:** the lockup stays
on screen across the `1450` boundary, in the same position and at the same scale,
...
version where the lockup exits at `1450` and the outro is only integrations plus a
URL loses the name at exactly the moment it is meant to stick.

Consequences the implementation must honour:

- there is no fade or cut at `1450` — the lockup simply does not move;
...
frame 1449 — unchanged from 1440; the frame the outro inherits
```
...
- Frame `1440` works as a still image: complete lockup, complete tagline, nothing
  mid-animation.
- The frame is static from `1440` to `1450`.
...
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="08 Brand" from={1360} durationInFrames={90}>` literals, because
...
- **Open decision — R3F or 2D.** Undecided. The geometry (one node, a handful of
  ...
  across the `1450` boundary into `09-outro.md`. Decide it against a rendered
  frame `1440`, and update `## Three.js` in the same task.
...
- **Open decision — lockup persistence across `1450`.** See
...
- **Sound absent.** `STORYBOARD.md` §18 asks for a *very soft impact* on the logo,
  around `1430`. Nothing is authored yet. Per §17 the sound may never be required
