...
- Global frames: `1090`–`1180`
- Scene-local frames: `0000`–`0090` (last rendered frame `0089` / master `1179`)
- Time: 18.17 s – 19.67 s
...
| Master        | Local         | Beat                                                     |
| ------------- | ------------- | -------------------------------------------------------- |
| `1090`        | `0000`        | Everything is gone. Black. Intentional silence.           |
| `1100`        | `0010`        | A single node appears.                                    |
| `1100`–`1140` | `0010`–`0050` | Thin lines arrive from outside the frame toward that node. |
| `1130`–`1160` | `0040`–`0070` | The relationships converge.                               |
| `1160`        | `0070`        | `Kivgraph` reads.                                         |
| `1170`        | `0080`        | `Exact code intelligence for coding agents.` reads.        |
| `1170`–`1180` | `0080`–`0090` | Settled. Nothing moves.                                   |

## Initial state

At `1090` the frame is empty and dark: `background` `#0a0b0d`, edge to edge.
Nothing else. The ten frames from `1090` to `1100` are deliberately empty — this
...
At `1179` the frame holds the settled Kivgraph lockup and its tagline, centred:
...
The single node at `1100` should be the mark itself: a small solid square in
...
**Arrival (`1100`–`1140`).** Each line is drawn from outside the frame toward the
...
**Convergence (`1130`–`1160`).** The lines complete, meet at the node, and then
...
**Naming (`1160` and `1170`).** The wordmark appears beside or beneath the mark,
...
This is a hard constraint here, because `1170` is a designated still-image key
frame in `AGENTS.md`. The tagline's entrance must therefore *conclude* at
`1170`, not begin there, and every line must be fully resolved before `1160`. If
anything is still moving at `1170`, the key frame is unusable.

Nothing moves between `1170` and `1180`.
...
- **Camera intent:** effectively static. A very small dolly-in (well inside the
  ...
  perfectly still by `1160`, because the frame is a logo.
...
- **Screen-space parity:** if 3D is chosen, the mark's final screen position and
  ...
  across the `1180` boundary. Matching a projected 3D position to a DOM layout
...
frame at `1090`. This scene then holds that emptiness for ten frames before the
node appears at `1100`.
...
At `1180` the composition continues into `09-outro.md`.

**Derived continuity decision, not literal in the storyboard:** the lockup stays
on screen across the `1180` boundary, in the same position and at the same scale,
...
version where the lockup exits at `1180` and the outro is only integrations plus a
URL loses the name at exactly the moment it is meant to stick.

Consequences the implementation must honour:

- there is no fade or cut at `1180` — the lockup simply does not move;
...
```text
frame 1090 — empty frame; the intentional silence
frame 1140 — relationships mid-arrival, converging on the mark
frame 1170 — complete lockup plus tagline, fully settled; STILL-IMAGE KEY FRAME
frame 1179 — unchanged from 1170; the frame the outro inherits
```

`1170` is on `AGENTS.md`'s still-image key frame list and `STORYBOARD.md` §28's
...
`1090`, `1140`, and `1170` are all on the manual review list in
`STORYBOARD.md` §28.
...
- Frame `1170` works as a still image: complete lockup, complete tagline, nothing
  mid-animation.
- The frame is static from `1170` to `1180`.
- The `1090`–`1100` emptiness is preserved.
...
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="08 Brand" from={1090} durationInFrames={90}>` literals, because
...
- **Open decision — R3F or 2D.** Undecided. The geometry (one node, a handful of
  ...
  across the `1180` boundary into `09-outro.md`. Decide it against a rendered
  frame `1170`, and update `## Three.js` in the same task.
...
- **Open decision — lockup persistence across `1180`.** See
...
- **Sound absent.** `STORYBOARD.md` §18 asks for a *very soft impact* on the logo,
  around `1160`. Nothing is authored yet. Per §17 the sound may never be required
