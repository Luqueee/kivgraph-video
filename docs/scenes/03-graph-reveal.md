...
SCENE 03 — FROM CODE TO GRAPH (0700–0770) and SCENE 04 — THE GRAPH EXPANDS (0770–1060). Both are
implemented by the single component `src/scenes/GraphRevealScene.tsx`, per `AGENTS.md` §7 and the
timeline in `STORYBOARD.md` §26, which already assigns one 380-frame `GraphRevealScene` sequence at
`from={480}`. Because of that merge this document sits one behind its storyboard numbers, and
...
- Storyboard scenes: `STORYBOARD.md` — SCENE 03 FROM CODE TO GRAPH **and** SCENE 04 THE GRAPH
  EXPANDS
- Global frames: 0700–1060
- Scene-local frames: 0000–0360
- Time: 11.67 s – 17.67 s
...
### Phase A — "from code to graph" (0700–0850 / local 0000–0150)
...
| Master | Local | Beat                                                                     |
| -----: | ----: | ------------------------------------------------------------------------ |
|  0700  |  0000 | `withRetry()` holds its screen position; the dissolve begins; camera held |
|  0730  |  0030 | substitution complete, the prompt layer is gone; the camera starts moving |
|  0762  |  0062 | the first edge begins to draw, leaving the node already on screen         |
|  0768  |  0068 | `Policy.Do()` begins to appear                                            |
|  0788  |  0088 | second edge begins                                                        |
|  0792  |  0092 | first edge complete                                                       |
|  0800  |  0100 | `Once()` begins; the camera reaches its second pose and turns downstream  |
|  0804  |  0104 | `Policy.Do()` fully present                                               |
|  0810  |  0110 | the anchor's plate has finished growing out of the prompt's field         |
|  0818  |  0118 | third edge begins                                                         |
|  0828  |  0128 | `Client.Charge()` begins; `Once()` fully present                          |
|  0850  |  0150 | Phase A ends; the move continues without a beat                           |

### Phase B — "the graph expands" (0850–1060 / local 0150–0360)
...
| Master | Local | Beat                                                                  |
| -----: | ----: | --------------------------------------------------------------------- |
|  0858  |  0158 | fourth edge begins                                                     |
|  0868  |  0168 | `Client.Refund()` begins; `Client.Charge()` fully present              |
|  1020  |  0320 | cluster label `payments-api` begins to surface                         |
|  1026  |  0326 | `Client.Refund()` settles; the near repository is complete             |
|  1058  |  0358 | `payments-api` label settled. **Nothing else happens for eighteen frames.** |
|  1076  |  0376 | one crossing begins, alone, travelling off the right of a frame the camera has not moved |
|  1075  |  0375 | the rig's hold ends; it starts opening out, following where the edge went |
|  1116  |  0416 | `checkout-service` label begins                                        |
|  1120  |  0420 | the first crossing lands; `CheckoutService.PlaceOrder()` begins        |
|  1126  |  0426 | second crossing begins                                                 |
|  1128  |  0428 | third crossing begins                                                  |
|  1134  |  0434 | `ReconciliationJob.Run()` begins                                       |
|  1136  |  0436 | `RefundHandler.Handle()` begins                                        |
|  1148  |  0448 | the camera reaches its final pose and holds                            |
|  1168  |  0468 | the last node settles, and the `checkout-service` label with it; hold  |
|  1180  |  0480 | scene ends; `04-blast-radius.md` continues the same graph              |

The phase boundary at 0770 is a label on this document, not an event in the shot. The camera has
been moving since 0730 and does not pause, change direction or change rate there. The viewer must
...
choreography: 318, 331 and 333 are the measured frames at which the camera has already carried each
...
`checkout-service` ramps over local 316–367, so it finishes on the frame its last plate,
...
At master frame 0700 (local 0000):

- scene 02's final image is on top at full opacity. Master `0699` and `0700` render
...
`(0.8316, -0.8845, 0)` to put its origin on `(1427, 733.82)`.
...
Frames 0699 and 0700 must be inspected side by side after any change to either scene.
...
| ----- | ----: | -----------: | ----------------------------------------------- |
| A     |  0300 |        32 px | the match cut; identical to the prompt token    |
| B     |  0415 |        39 px | direct callers present                          |
| C     |  0500 |        49 px | the near repository complete, camera closest    |
| D     |  0552 |        31 px | the crossings in flight                         |
| E     |  0599 |        29 px | the whole graph, settled                        |

- Tightest label margin over all 380 frames: **55 px**, on `checkout-service` at local 322.
...
At master frame 1060 (local 0360):

- camera at eye `(7.0, 3.2, 10.0)` looking at `(8.0, 0.0, -2.4)`, held since local 348;
...
reads this scene's settled values through `getGraphState(380)`, because every ramp in this scene
clamps and 380 therefore returns what frame 1059 renders.
...
**The dissolve and the substitution (0700–0730 / local 0000–0030).** The prompt layer and the
...
```text
   local 0000-0030   eye (0.0, 0.0,  9.0)  ->  (0.0,  0.0,  0.0)   hold, the match cut
   local 0030-0100   eye (0.6, 0.4,  7.4)  ->  (1.3, -0.4, -0.4)   step in, off the axis
   local 0100-0195   eye (2.6, 0.2,  6.2)  ->  (3.8, -0.6, -1.0)   travel with the impact
   local 0195-0255   eye (2.6, 0.2,  6.2)  ->  (3.8, -0.6, -1.0)   held, while the crossing draws
   local 0255-0328   eye (7.0, 3.2, 10.0)  ->  (8.0,  0.0, -2.4)   rise and open out, after it
   local 0328-0360   held

Scene-local, read straight off the keyframe list in `src/three/graphState.ts`, so
a retime cannot make this table wrong. Master is local + the scene's start.
```
...
the pass itself as built, measured and retired. Frame 1059 lands without it.
...
Hard boundary at 0700 in `src/Composition.tsx`, but **not a visual cut**. Scene 02 delivers a
contract at its frame 0699: the screen-space centre and apparent scale of the token `withRetry()`,
...
- The central node occupies that same screen rectangle at 0700.
...
- The prompt layer and the residual code world are dissolved by **this** scene, over 0700–0730, not
  by scene 02.

If `promptLayout`, the token's size or the push-in curve changes in `02-agent.md`, this scene must be
updated in the same task. Frames 0699 and 0700 are always inspected as a pair.

## Transition out

Hard boundary at 1060 into `04-blast-radius.md`, but **not a visual cut**. The next scene continues
...
The graph state at frame 1059 must therefore equal the graph state scene 04 assumes at 1060. The
...
```text
frame 0700 — match cut; withRetry() in its prompt position, the layer beginning to dissolve
frame 0730 — the word is now a 3D node, fully lit and accented; the prompt is gone; the code is
             still legible underneath
frame 0810 — the anchor's plate has finished forming; two callers present; one edge resolving
frame 0900 — the near repository is complete and begins to name itself
frame 1000 — a crossing edge is in flight toward empty frame; the reveal is a camera move
frame 1059 — the whole graph, settled — KEY VISUAL, must work as a still image
```

`STORYBOARD.md` §28 lists 0700, 0730, 0760, 0820, 0770, 0910, 1020 and 0970 for manual review. The
list above is where the beats actually land now, and the two should be reconciled the next time the
storyboard is touched.

**Frame 1059 is the key visual frame** (`STORYBOARD.md` §29), intended for social use. As a still,
...
- **The match cut at 0700 is not covered by an effect.** No flash, no whip pan, no blur transition.
- **Accent marks exactly one thing at a time:** the anchor's hairline, and an edge while it is
  resolving.
- **The graph state is continuous across 1060.** Scene 04 must be able to assume it.
...
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="03 Graph Reveal" from={700} durationInFrames={360}>`. That file is the single
...
- **Phase structure.** This component covers two storyboard scenes but is a single flat 380-frame
...
- Derived state: `getGraphState(frame)` in `src/three/graphState.ts`, returning the
  ...
  function is what makes the causal ordering auditable and the 1060 handoff verifiable.
...
- Performance: 1920×1080 at 60 fps for 380 frames. Geometry is rebuilt per frame for eight plates
...
- **Sound is absent.** `STORYBOARD.md` §18 suggests a small tick on edge resolve and a slightly
  deeper sound on the cross-repository connection. Neither is implemented, and frame 1059 must land
  without them.
...
- **`STORYBOARD.md` §28–§29 still list 0970** as the key visual, and the last node now settles at
  1047 with the scene holding to 1060. `1059` is the frame that ships; the storyboard should be
