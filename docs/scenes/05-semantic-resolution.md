...
**Immediately before.** Scene 04 (`04-blast-radius.md`, frames 0870–1010)
...
**What it prepares.** Scene 06 (`06-agent-answer.md`, frames 1210–1390) returns
...
token — closing the loop the 0510 cut opened — is Scene 06's move, made in its own
hundred and eighty frames. This scene owns the contraction that leaves the
symbol alone and must have it fully settled at 1209.
...
- Global frames: **1010–1210**
- Scene-local frames: **0180–0380**
...
| Master      | Local       | Beat                                                                                             |
| ----------- | ----------- | ------------------------------------------------------------------------------------------------ |
| `1010–1058` | `0000–0048` | The rig straightens, depth collapses, the graph flattens into the right half, split view forms. The divider draws over local 14–52; the impact card fades out over local 0–36. |
| `1044–1100` | `0034–0090` | Left side builds: label `Name matching` (local 34–54), the two `withRetry()` entries under two languages (42–62 and 54–74), both highlighted, then the counter (70–90). |
| `1074–1108` | `0064–0098` | Right side resolves: label `Semantic resolution` (local 64–84), the symbol and the two relationships that reach it, then the counter (78–98). |
| `1108`      | `0098`      | Both counters settled: `2 matches` left, `1 symbol` and `2 real relationships` right. The comparison is complete, and nothing leaves it for sixty frames. |
| `1104–1168` | `0094–0158` | The stand: 65 frames byte-identical, both halves at full presence. `1104` is the key visual frame and is the first frame of this run. |
| `1168–1196` | `0158–0186` | One exit window, twenty-eight frames: the two callers and their tubes, the left column, the divider, `Semantic resolution` and both counters leave together. |
| `1196–1209` | `0186–0199` | `withRetry()` alone, pixel-identical through to 1209 for the match cut. |
...
The internal beat frames between 1010 and 1108 are this document's proposal; the
...
copy at what is now 1104 was cut, so the key visual frame is the comparison itself
rather than a sentence over it. See `## Flexible elements` and
`## Current compromises`.

## Initial state

At master frame 1010 (local 0180) the frame is **visually identical** to master
frame 1009: the 3D graph as Scene 04 left it, at rest and accent-marked, at full
...
At master frame 1210 (local 0380, the frame after this scene's last):

By 1209 the frame holds **one symbol and the code bed underneath it**, and
nothing else:
...
- the code bed, dim, exactly as scene 04 left it. It has been under every frame
  since 0180 and it carries on into scene 06, which is what makes the cut a
...
as a comparison" at 1209. That was written when the scene ended *on* the
...
Nothing is mid-animation at 1209: the frame is pixel-identical from 1196, fourteen
...
**Hierarchy** at 1104, strongest first:
...
`Easing.bezier(0.22, 1, 0.36, 1)` across `0770–0818`. The impact card fades out
...
after the fact: that the still at 1104 needed the asymmetry to carry it. It does
...
of frame 1168: left `87 / 53 / 18 / 8`, the right column's label
`88 / 54 / 20 / 8`.
...
**The stand.** The comparison is complete at 1108 and nothing leaves it until
1168 — sixty frames, a full second, which is the reading time two columns need and
the counters' whole dwell. With no dim inside it the frame is byte-identical for 65
frames, master `0864–0928`, and both halves are at full presence for all of them.
...
level of eight-bit grey; `1104` is the measured start, which is why it is the key
frame rather than `1108`.

**The contraction.** Everything leaves on one window, local 158–186 (master
`0928–0956`): the two callers and their tubes, and with them the left column, the
...
before the cut, so the frame is pixel-identical from 1196 to 1209 and the shape is
...
had looked straight down `-Z` since 0510; it never did. The camera is a rig — an
...
has built since frame 0510; flattening the actual graph proves that the
...
Hard sequence boundary at 1010, and it is **pixel-identical**: at 1010 the frame
equals 1009 — the accent-marked graph at rest at full presence, with the impact
...
places, and the 1009/1010 seam measured 22 dB. Both scenes now draw
...
- the graph must be **at rest** at 1010, otherwise the flatten reads as a glitch
...
At 1210 `AgentAnswerScene.tsx` takes over with a **match cut** into the agent
...
- this scene owns the contraction, which is everything leaving on one window over
  local 158–186, and completes it at **1196**;
- from 1196 to 1209 the frame is pixel-identical: `withRetry()` alone, settled and
  still, at the centre of the right column and at the apparent size the column
  drew it at;
- at 1210 Scene 06 opens on the symbol in that same place at that same size, and
...
the 0510 cut opened — `graphOffset` in `src/three/graphFrame.ts` is derived from
...
which is why nothing about the surviving node moves after 1168 and why the exit
...
frame from what is now 1104; it was cut, and nothing replaced it.
...
```text
1010 — frame identical to 1009; flatten begins
1070 — split view established, both matches highlighted, right side resolving
1104 — KEY VISUAL: name matching vs semantic resolution, the asymmetry alone
1209 — contraction settled since 1196, ready for the match cut
```

**Frame 1104 is a key visual frame** (`STORYBOARD.md` §29 and `AGENTS.md`'s
...
that run now begins. It must work as a still image, which means at 1104:
...
- **The transition into the split comes from the content.** The graph flattens out
  ...
  (`STORYBOARD.md` §27).
- **The contraction is a subtraction and it is settled at 1196, fourteen frames
...
- **Frame 1104 works as a still image.**
...
- exact frames of the left build and the right resolve, provided both are complete
  and both counters are legible by roughly 1108, provided no build window is
...
- Global scene boundaries live only as inline `<Sequence>` literals in
  ...
  restate 1010/1210 inside the scene. The scene animates in scene-local frames.
...
- Tokens: `src/brand/tokens.ts`. Fonts: `src/brand/fonts.ts` (`"Geist"`,
  ...
  frame.
- The hand-over at 1210 is a cross-component contract, and it is a position and an
  ...
  `graphOffset` in `src/three/graphFrame.ts` was derived from at the 0510 cut, so
  the return closes that loop rather than repeating it. Inspect 1209 and 1210 as a
...
- Performance: the flatten animates camera plus per-node depth and opacity while a
  ...
  (`AGENTS.md` §35, §36).
- Iterate with Remotion Studio plus small range renders over roughly 1000–1220.
  Scrub forward, backward, forward across both seams (1009/1010 and 1209/1210) and
...
- **Ownership of the contraction is an open decision resolved here.**
  ...
  the build completing at `1108`, the sixty-frame stand, one exit window
  `0928–0956` and the settle to the cut — while SCENE 08 still places "el grafo se
...
- **Internal beat frames are this document's proposal.** The storyboard fixes the
  transition, the build to `1108`, the stand, the exit window `0928–0956` and the
  settle to the cut; its centred copy at what is now `1104` was cut, so that beat
  no longer constrains anything. The flatten window, the left build and the right
  resolve windows in `## Timeline` are derived from the critical frames `1010`,
  `1070`, `1104` and `1209`.
