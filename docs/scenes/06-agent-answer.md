...
- Global frames: `1210`–`1390`
- Scene-local frames: `0180`–`0360` (last rendered frame `0359` / master `1389`)
...
| Master        | Local         | Beat                                                             |
| ------------- | ------------- | ---------------------------------------------------------------- |
| `1210`        | `0000`        | Match cut lands. Prompt layer is back, question still on screen.  |
| `1210`–`1234` | `0000`–`0024` | The symbol travels 201 px left and 74 px up to the token slot.    |
| `1234`–`1240` | `0024`–`0030` | Cross-fade: the travelling symbol becomes the prompt's token.     |
| `1240`–`1256` | `0030`–`0046` | Block 1: `Changing withRetry() affects:`                          |
| `1256`–`1272` | `0046`–`0062` | Block 2: `7 symbols` / `across 2 repositories.`                   |
| `1274`–`1290` | `0064`–`0080` | Block 3: the `checkout-service` path sentence                     |
| `1290`–`1304` | `0080`–`0094` | Label `Answered with Kivgraph` arrives and reads.                 |
| `1304`–`1390` | `0094`–`0180` | Hold. Nothing moves; the render measures 87 frames identical from `1303`. The frame is settled for the hard cut, and the hold is where the answer is read. |

Only `1210` and `1304` are fixed. The three block windows are the documented
...
`1304`, and no block loses dwell time against the figures in `## Motion` →
**The hold, and why the scene is 180 frames**.

## Initial state

At `1210` the frame is the agent prompt layer from `02-agent.md`, at the same
...
- the graph reduced to that single travelling symbol, drawn by the same renderer
  that drew it at `1209` so the cut matches; no other residue of the split view,
...
At `1389` the prompt layer holds the complete answer:
...
unchanged; at 26 px on one line it measured 1373 px, 62 % of the frame, and would
...
question reads. By `1210` the code bed is down at `0.02` and the answer is the
...
Measured on the settled frame `1304`: the content ran `844` to `1216`, so its
centre sat at `1030` against the frame's `720`. Lifted, it runs `497` to `988`,
centre `712` — deliberately a little above geometric centre, which is the
...
content is left-aligned at `680` under a rule that starts at `620`, so its
horizontal centre is a property of the rule and the longest line rather than
something to tune. It measures `1129` today, 71 px left of the frame's — 3.7 %,
...
the camera path and the key stills `0869`, `0958` and `1104` with it. The lift is
...
*finishes* on `1304` rather than beginning there, so the reviewed frame `1304`
shows it legible.
...
Nothing moves between `1304` and `1390`.
...
frame earlier at `1303` because the label's easing is front-loaded and its last
...
cut at `1210` lands on a shape that is already exactly there. Carrying that shape
...
Because Remotion `Sequence` boundaries at `1210` do not overlap, the contraction
is authored as the **tail of `SemanticScene`**, complete at frame `1196` and at
rest through `1209`. A match cut requires the outgoing shape to be settled at the
cut; if the contraction were still running at `1210` there would be nothing to
match. Agreed division with `05-semantic-resolution.md`: the comparison stands
`1108`–`1168`, everything leaves on one window `1168`–`1196`, and the frame is
pixel-identical from `1196` to `1209`. See `## Current compromises` for the
...
Hard cut at `1390` into `07-benchmark.md`.
...
Kivgraph`. It is on screen at `1390` at exactly the position, size, colour and
letter-spacing it holds at `1389` — the region measures `inf` PSNR across the
...
blocks and every pixel of the terminal are gone at `1390`, and the whole frame
...
Carried over from `02-agent.md` and visible from frame `1210`, not new copy:
...
```text
frame 1210 — match cut lands; prompt restored, question still on screen, no answer yet
frame 1000 — token landed, code bed back up, first block arriving
frame 1304 — full answer plus the "Answered with Kivgraph" label; the resolved frame
frame 1389 — settled final frame immediately before the hard cut
```

`1210` and `1304` are on the manual review list in `STORYBOARD.md` §28. `1304` is
...
still-image key frame list is `0260`, `0869`, `0958`, `1104`, `1304`, `1470`,
`1630` — `1304` is on it because this retime put it there), but `1304` is the
...
- The prompt layer reappears at the **same scale** it had in `02-agent.md`, read
  ...
  prompt is not on screen at `1209` and there is nothing for it to match. If the
...
- **The lift reaches the travelling symbol through the camera pose, never
  ...
  should therefore have left `1210` untouched — and did not: a transform
  resamples the WebGL texture whether or not it moves anything, and `1210` came
...
- **`Attribution` is rendered outside the lift wrapper.** Its `y` already carries
  ...
  the cut at `1389`/`1390`. Applying the wrapper to it as well would offset it
...
- Accent marks only the result and the Kivgraph invocation. The scene stays
  ≥ 85 % neutral.
- The frame is static from `1304` to `1390`, and that hold is the answer's reading
...
- **Storyboard tension, resolved.** `STORYBOARD.md` places the graph contraction
  ...
  stands complete to `1168` and then leaves on one window `1168`–`1196`. Both
  cannot be true at a non-overlapping Sequence boundary, and a contraction that
  begins at `1210` is unreachable inside `AgentAnswerScene` without making this a
  3D scene. Resolution agreed with `05-semantic-resolution.md`: `SemanticScene`
  owns the contraction — the stand `1108`–`1168`, the exit `1168`–`1196`, and the
  frame pixel-identical from `1196` to `1209`. Both documents record the same
