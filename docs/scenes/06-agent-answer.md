...
- Global frames: `1030`–`1210`
- Scene-local frames: `0000`–`0180` (last rendered frame `0179` / master `1209`)
...
| Master        | Local         | Beat                                                             |
| ------------- | ------------- | ---------------------------------------------------------------- |
| `1030`        | `0000`        | Match cut lands. Prompt layer is back, question still on screen.  |
| `1030`–`1054` | `0000`–`0024` | The symbol travels 201 px left and 74 px up to the token slot.    |
| `1054`–`1000` | `0024`–`0030` | Cross-fade: the travelling symbol becomes the prompt's token.     |
| `1000`–`1076` | `0030`–`0046` | Block 1: `Changing withRetry() affects:`                          |
| `1076`–`1092` | `0046`–`0062` | Block 2: `7 symbols` / `across 2 repositories.`                   |
| `1094`–`1110` | `0064`–`0080` | Block 3: the `checkout-service` path sentence                     |
| `1110`–`1124` | `0080`–`0094` | Label `Answered with Kivgraph` arrives and reads.                 |
| `1124`–`1210` | `0094`–`0180` | Hold. Nothing moves; the render measures 87 frames identical from `1123`. The frame is settled for the hard cut, and the hold is where the answer is read. |

Only `1030` and `1124` are fixed. The three block windows are the documented
...
`1124`, and no block loses dwell time against the figures in `## Motion` →
**The hold, and why the scene is 180 frames**.

## Initial state

At `1030` the frame is the agent prompt layer from `02-agent.md`, at the same
...
- the graph reduced to that single travelling symbol, drawn by the same renderer
  that drew it at `1029` so the cut matches; no other residue of the split view,
...
At `1209` the prompt layer holds the complete answer:
...
question reads. By `1030` the code bed is down at `0.02` and the answer is the
...
Measured on the settled frame `1124`: the content ran `664` to `1036`, so its
centre sat at `850` against the frame's `540`. Lifted, it runs `317` to `808`,
...
something to tune. It measures `949` today, 71 px left of the frame's — 3.7 %,
...
the camera path and the key stills `0689`, `0778` and `0924` with it. The lift is
...
*finishes* on `1124` rather than beginning there, so the reviewed frame `1124`
shows it legible.
...
Nothing moves between `1124` and `1210`.
...
frame earlier at `1123` because the label's easing is front-loaded and its last
...
cut at `1030` lands on a shape that is already exactly there. Carrying that shape
...
Because Remotion `Sequence` boundaries at `1030` do not overlap, the contraction
is authored as the **tail of `SemanticScene`**, complete at frame `1016` and at
rest through `1029`. A match cut requires the outgoing shape to be settled at the
cut; if the contraction were still running at `1030` there would be nothing to
match. Agreed division with `05-semantic-resolution.md`: the comparison stands
`0928`–`0988`, everything leaves on one window `0988`–`1016`, and the frame is
pixel-identical from `1016` to `1029`. See `## Current compromises` for the
...
Hard cut at `1210` into `07-benchmark.md`.
...
Kivgraph`. It is on screen at `1210` at exactly the position, size, colour and
letter-spacing it holds at `1209` — the region measures `inf` PSNR across the
...
blocks and every pixel of the terminal are gone at `1210`, and the whole frame
...
Carried over from `02-agent.md` and visible from frame `1030`, not new copy:
...
```text
frame 1030 — match cut lands; prompt restored, question still on screen, no answer yet
frame 1000 — token landed, code bed back up, first block arriving
frame 1124 — full answer plus the "Answered with Kivgraph" label; the resolved frame
frame 1209 — settled final frame immediately before the hard cut
```

`1030` and `1124` are on the manual review list in `STORYBOARD.md` §28. `1124` is
...
still-image key frame list is `0080`, `0689`, `0778`, `0924`, `1124`, `1290`,
`1450` — `1124` is on it because this retime put it there), but `1124` is the
...
- The prompt layer reappears at the **same scale** it had in `02-agent.md`, read
  ...
  prompt is not on screen at `1029` and there is nothing for it to match. If the
...
- **The lift reaches the travelling symbol through the camera pose, never
  ...
  should therefore have left `1030` untouched — and did not: a transform
  resamples the WebGL texture whether or not it moves anything, and `1030` came
...
- **`Attribution` is rendered outside the lift wrapper.** Its `y` already carries
  ...
  the cut at `1209`/`1210`. Applying the wrapper to it as well would offset it
...
- Accent marks only the result and the Kivgraph invocation. The scene stays
  ≥ 85 % neutral.
- The frame is static from `1124` to `1210`, and that hold is the answer's reading
...
- **Storyboard tension, resolved.** `STORYBOARD.md` places the graph contraction
  ...
  stands complete to `0988` and then leaves on one window `0988`–`1016`. Both
  cannot be true at a non-overlapping Sequence boundary, and a contraction that
  begins at `1030` is unreachable inside `AgentAnswerScene` without making this a
  3D scene. Resolution agreed with `05-semantic-resolution.md`: `SemanticScene`
  owns the contraction — the stand `0928`–`0988`, the exit `0988`–`1016`, and the
  frame pixel-identical from `1016` to `1029`. Both documents record the same
