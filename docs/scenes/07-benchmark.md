...
Everything up to `1390` was demonstration: a graph, its cross-repository edges,
...
| `grep + read` | `267,980` | `281` |   `1.000` | `0.98851` | `28/29` | `29/29`  |
...
- Global frames: `1390`–`1640`
- Scene-local frames: `0180`–`0430` (last rendered frame `0429` / master `1639`)
...
| Master        | Local         | Beat                                                                     |
| ------------- | ------------- | ------------------------------------------------------------------------ |
| `1390`        | `0000`        | Hard cut. Empty background.                                              |
| `1392`–`1412` | `0002`–`0022` | The two column heads, `kivgraph` and `grep + read`, arrive together.     |
| `1400`–`1420` | `0010`–`0030` | The hairline rule under the heads, to `0.9` opacity.                     |
| `1404`–`1428` | `0014`–`0038` | The cost row, `tokens`: `35,961` at 76 px, `267,980` at 36 px and faint. |
| `1432`–`1452` | `0042`–`0062` | The `exact answers` row: `28 / 29` in both columns.                      |
| `1456`–`1476` | `0066`–`0086` | The `precision` row: `1.000` in both columns.                            |
| `1480`–`1500` | `0090`–`0110` | The `recall` row: `0.996` against `0.989`.                               |
| `1506`–`1526` | `0116`–`0136` | The source note, `37 repositories · published benchmark`, lands last.    |
| `1526`–`1588` | `0136`–`0198` | Settled. The render measures 63 byte-identical frames.                   |
| `1588`–`1600` | `0198`–`0210` | The whole table fades out together.                                      |
...
Only the hard cut at `1390` and the still-image key frame at `1470` are fixed by
...
Master timeline: **1850 frames, 26.17 s**. Nothing before `1390` moved: scenes
01–06 are untouched. Scene 08 is `1640`–`1730` and scene 09 is `1730`–`1850`.

## Initial state

At `1390` the frame is empty: `background` `#0a0b0d`, edge to edge. No panel, no
...
At master `1627`, the last frame before the fade, the table is complete and
static:
...
The render measures this frame byte-identical for **63 frames**, master `1566`–
`1628`. A viewer landing on any one of those 63 frames should be able to
...
- `labelLeft: 560`, `columnRight: [1300, 1640]` — 800 px wide, centred in the
  1920 frame.
...
The block runs `522`–`974` and is centred on **538**, not 540: mono digits sit
...
Measured on the render at settled frame `1625`: column A right-aligns at exactly
`x 1057` and column B at `x 1357` — 3 px of mono side bearing inside the declared
column edges — the block's ink spans `x 560`–`1639` for a centre of **959.5**
against a frame centre of 960, and the rule spans `x 560`–`1639` at `y 388`,
...
| ------------------------- | ----------- | ------------- |
| the two column heads      | `182`–`202` | `1392`–`1452` |
| the hairline rule         | `190`–`210` | `1400`–`1460` |
| the cost row, `tokens`    | `194`–`218` | `1404`–`1468` |
| the `exact answers` row   | `222`–`242` | `1472`–`1492` |
| the `precision` row       | `246`–`266` | `1496`–`1516` |
| the `recall` row          | `270`–`290` | `1520`–`1540` |
| the source note           | `296`–`316` | `1546`–`1566` |
| fade out, the whole table | `378`–`390` | `1628`–`1640` |
...
and frame `1470` is the case where that matters. It is a designated still-image
key frame (`STORYBOARD.md` §29). The cost row therefore completes at local `218`
/ master `1468`, and the `exact answers` row does not begin until `1472`, so
`1470` carries `35,961` and `267,980` settled under their two named arms with
nothing half-arrived beneath them. A half-faded figure at `1470` would ruin the
...
Measured on the 210-frame cut, with the fade beginning at master `1628`:

| Element                 | Settles at master |               Dwell | Chars/s |
| ----------------------- | ----------------: | ------------------: | ------: |
| column heads            |            `1452` | 176 frames / 2.93 s |     6.5 |
| the `tokens` row        |            `1468` | 160 frames / 2.67 s |     7.1 |
| the `exact answers` row |            `1492` | 136 frames / 2.27 s |    11.9 |
| the `precision` row     |            `1516` | 112 frames / 1.87 s |    10.2 |
| the `recall` row        |            `1540` |  88 frames / 1.47 s |    10.9 |
| the source note         |            `1566` |  62 frames / 1.03 s |    35.8 |
...
The hold from master `1566` to `1628` is not padding and it is not a settled
...
The scene is fully static from `1566`, so the fade toward `08-brand.md` begins
...
Hard clean cut at `1390` (`STORYBOARD.md`: *hard cut limpio*).
...
arrives already on screen at `1390`, at exactly the position, size, colour and
...
not the three answer blocks. `STORYBOARD.md`'s frame spec for `1390` still holds
...
Scene 06 is byte-identical for its last 87 frames, `1303`–`1389`, and that
...
builds at `y 342`–`974`, so for a third of a second there was nothing to look at
anywhere.

Carrying the attribution puts something at `1390` in the place the eye already
...
The whole **table** fades out together over `1628`–`1640`, ending on the empty
brand background at `1640`. It fades as one composition; fading the rows out in
...
behind it. The frame reached pure `#000000` at `1636` and held it to `1639`, and
scene 08 then restored `#0a0b0d` at `1640` — a ten-level step on a flat frame,
...
`mountedFrames` was 1640 and the film ended here.

The fade now lives on an inner fill under an opaque one. Only `1628`–`1639`
changed: `fadeOut` is 1 for every frame before local `378`, so `1470`, `1580` and
`1627` are byte-identical to the render before the fix, and nothing about the
`1390` cut or the 63-frame settled run moved. **Never put this scene's fade back
on the element that carries `backgroundColor`.**

Measured after the fix: `1639` and `1640` are pixel-identical, `PSNR = inf`, and
the corner holds `10 11 13` across the boundary. The fade actually lands two
frames early, at `1638`, because the project's easing is within one 255th of its
final value before its ramp ends — so the scene's own tail and scene 08's ten
frames of silence form one twelve-frame identical run, `1638`–`1649`.

`1640`–`1650` is that silence, and it is not a hole in the timeline: it is scene
08's own opening beat, its scene-local `0180`–`0190`, and `08-brand.md` owns it.
...
```text
frame 1390 — hard cut; empty frame, nothing yet
frame 1470 — column heads, rule and the cost row all complete; `35,961` and
             `267,980` legible under `kivgraph` and `grep + read`, the three
             correctness rows not yet arrived; STILL-IMAGE KEY FRAME
frame 1530 — heads, rule, `tokens`, `exact answers` and `precision` complete,
             `recall` mid-arrival
frame 1580 — the settled table, inside the 63-frame identical run 1566-1348
frame 1627 — the last frame before the fade
```

`1470` is on both `AGENTS.md`'s still-image key frame list and `STORYBOARD.md`
...
- **The shared right edge, per column.** Column A right-aligns on
  `tableGrid.columnRight[0]` `1300` (measured ink at `x 1057`) and column B on
  `columnRight[1]` `1640` (measured at `x 1357`). The comparison is made by the
...
- Frame `1470` works as a still image, with `35,961` and `267,980` fully legible
  and both arms named above them.
- The hard cut at `1390` stays hard.
- The scene is static from `1566` to `1628`, and the whole table leaves together.
...
- The exact window starts, provided no element loses dwell against the table in
  `## Motion` and the cost row is complete at `1470`.
...
- Component: `src/scenes/BenchmarkScene.tsx`. It owns the layout — four entries
  in one `layout` constant (`headerTop` `522`, `ruleY` `568`, `rowTops`
  `[416, 516, 576, 636]`, `noteTop` `956`) — and the timing: the header `entry`,
...
- `tableGrid` is `{ labelLeft: 560, columnRight: [1300, 1640] }`, exported
...
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="07 Benchmark" from={1390} durationInFrames={210} premountFor={30}>`
  ...
  `mountedFrames` is `1640`, this scene's end boundary.
...
- Measured on the render: **1640 frames with no black frame**, the settled table
  byte-identical for 63 frames (`1566`–`1628`), and the only single-frame
  anomalies at the two hard cuts, `1209`/`1210` and `1389`/`1390`. Seams:
  `0869`/`0870` at **62.93 dB** and `1009`/`1010` pixel-identical, both unchanged
  by this figures pass; `1209`/`1210` at **30.44 dB whole frame**, a match cut,
  and a whole-frame figure is not comparable to a symbol-region one so always
  say which is quoted; `1389`/`1390` at **24.25 dB**, where a hard cut is
...
- **The `1470` still names both arms now, but still sits above frame centre.**
  ...
  the block is laid out for four rows and the lower three are empty at `1470`.
