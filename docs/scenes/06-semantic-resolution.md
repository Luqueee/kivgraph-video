# Scene 06 — Semantic Resolution

## Purpose

This scene proves the product claim.

Scene 05 asserted `Exact symbols. Not name matches.` An assertion is not
evidence. This scene supplies the evidence by putting the two approaches side by
side and letting the viewer see the difference without being told: searching a
name returns whatever happens to be spelled that way; resolving a symbol returns one thing and its
real relationships.
...
from `STORYBOARD.md` §3, and it carries the video's secondary brand message,
`A name is not a symbol.` (`STORYBOARD.md` §4).

This is also the scene that makes everything before it credible. Without it, the
blast radius from Scene 05 could plausibly be a fancy text search. With it, the
...
**Immediately before.** Scene 05 (`05-blast-radius.md`, frames 0720–0840)
propagated a change through the graph, counted it on a card
(`7 affected symbols`, `3 dependency paths`, `2 repositories`) and stated
`Exact symbols. Not name matches.` The graph is accent-marked and at rest.
...
**What it prepares.** Scene 07 (`07-agent-answer.md`, frames 0990–1080) returns
...
`withRetry()` alone in the right column, settled and still, and Scene 07 opens on
it in that same place at that same apparent size. Carrying it back to the prompt's
token — closing the loop the 0330 cut opened — is Scene 07's move, made in its own
ninety frames. This scene owns the contraction that leaves the symbol alone and
must have it fully settled at 0989.
...
- Storyboard coverage: **`STORYBOARD.md` SCENE 07 — NAME ≠ SYMBOL**.
  ...
  component `GraphRevealScene`, so every document number from 04 onward is one
  lower than its storyboard scene number.
- Global frames: **0840–0990**
- Scene-local frames: **0000–0150**
- Time: **14.0 s – 16.5 s**
- Duration: **150 frames / 2.5 s** at 60 fps
- Remotion component: `src/scenes/SemanticScene.tsx`
- Boundary source: the inline `<Sequence name="06 Semantic Resolution" from={840} durationInFrames={150}>`
  literal in `src/Composition.tsx`

Beats (scene-local = master − 840):

| Master      | Local       | Beat                                                                                             |
| ----------- | ----------- | ------------------------------------------------------------------------------------------------ |
| `0840–0880` | `0000–0040` | The rig straightens, depth collapses, the graph flattens into the right half, split view forms. Inherited veil comes off. Card and previous copy fade out. |
| `0870–0910` | `0030–0070` | Left side builds: label `Name matching`, two `withRetry()` entries under two languages, both highlighted. |
| `0895–0925` | `0055–0085` | Right side resolves: label `Semantic resolution`, the symbol and the two relationships that reach it. |
| `0925`      | `0085`      | Both counters settled: `2 matches` left, `1 symbol` and `2 real relationships` right.              |
| `0930–0970` | `0090–0130` | Left side loses opacity. Right side stays clear.                                                   |
| `0936–0952` | `0096–0112` | Second veil rises to `0.58`, then `A name is not a symbol.` fades in; complete before 0950, the key visual frame. |
| `0970–0979` | `0130–0139` | Hold.                                                                                              |
| `0979–0989` | `0139–0149` | The two callers and their tubes leave; `withRetry()` alone, settled and still at 0989 for the match cut. |

The internal beat frames between 0840 and 0925 are this document's proposal; the
storyboard fixes only the transition, the `0930–0970` fade, the copy at `0950` and
the hold. See `## Flexible elements` and `## Current compromises`.

## Initial state

At master frame 0840 (local 0000) the frame is **visually identical** to master
frame 0839: the 3D graph as Scene 05 left it, at rest and accent-marked, dimmed
under Scene 05's veil at `0.72`, with the impact card in the **top-left** and
`Exact symbols. Not name matches.` below it, both above the veil and at full
presence.

The inherited veil is this scene's to spend, and spending it means taking it off.
`STORYBOARD.md` § Frase sobre el cuadro requires a viewer-addressed sentence to be
read on a darkened frame and the veil to return to zero inside the scene that
raised it *unless the next scene's document says it inherits one* — this is that
statement, so Scene 05's `0.72` comes off here, across the flatten. It has to: a
comparison is read rather than glanced at, and both columns have to be lit
identically for the asymmetry between them to read as data rather than as
emphasis. The flatten also begins from an already-quiet frame, so the change of
representation does not have to fight the graph's own luminance.

`A name is not a symbol.` then needs a darkened frame of its own and gets one: a
second veil, rising to `0.58` over local 96–112 rather than Scene 04's `0.86`,
because the comparison is the evidence for the sentence and has to survive
underneath it. Two veils in one scene needs justifying rather than assuming, and
the justification is the ninety frames of legible comparison between them — that
is a scene, not a flicker. The rule that survives is the narrower one: **one veil
per sentence, never two inside twenty frames.** Both live in one expression,
`veilOpacity` in `src/three/semanticState.ts`, so the lift and the return cannot
be authored out of step.
...
At master frame 0990 (local 0150, the frame after this scene's last):

By 0989 the frame holds:
...
- `A name is not a symbol.` still on screen;
- the two callers and their tubes gone, leaving the single `withRetry()` node
  ...
  the size `AgentAnswerScene.tsx` opens on at 0990.

Nothing is mid-animation at 0989. A match cut requires the outgoing shape to be at
rest at the cut.
...
A horizontal band across the middle-lower area of the frame is **reserved from the
start** for `A name is not a symbol.` Both columns are composed around that band
from frame 0840 onward, so that when the copy arrives at 0950 nothing has to move
aside. Layout must never reflow on a key frame.
...
**The centre copy.** `A name is not a symbol.` in Geist sans (the project rule:
graph-attached text is mono, viewer-addressed narrative is sans), weight 500,
`textPrimary` `#f5f5f5`, Hero scale (72–92 px per `STORYBOARD.md` §7), centred
horizontally across the whole frame — it belongs to neither column because it is
the conclusion drawn from both.
...
**Hierarchy** at 0950, strongest first:

1. `A name is not a symbol.`;
2. the right side's single resolved symbol;
3. the left side's two matches, already fading;
4. the counters;
5. the side labels and the divider.

## Motion

**Rhythm.** Flatten, build, compare, subtract, state, hold, and leave one symbol.
The scene is 2.5 s long — the longest of this trio — because the comparison needs
time to be counted, not just seen.
...
`Easing.bezier(0.22, 1, 0.36, 1)` across `0840–0880`. The card and
`Exact symbols. Not name matches.` fade out during this move; they do not slide
or wipe.
...
05's strictly ordered causal propagation: name matching dumps results in no
...
**The subtraction.** `0930–0970` reduces the left side's opacity while the right
...
in the still at 0950.

**The copy.** A short fade at 0950, no movement, no scale-in. It arrives once the
asymmetry is already visible, so it reads as a conclusion rather than a caption.

**The hold.** `0970` onward the frame is still and readable.
...
07, which has ninety frames for it; doing it here would mean two moves at once in
ten frames, one of them a snap, under a sentence already on screen and holding.
...
`(8.0, 0.0, -2.4)`: roughly 15° off `-Z`. Scene 04 translates the eye and the
target together, so the view direction never changes, and returns to that pose;
Scene 05 never moves it. The pose is exported as `restLook` from
`src/three/crossRepoState.ts`, so this scene reads what it inherits instead of
restating it. Straightening the rig to frontal is therefore part of the flatten,
...
- the rig straightens from `restLook` to a pose whose view direction is exactly
...
- the left column, both counters, the divider and the sentence are DOM,
...
per-edge gains, both veils and the final contraction are all `state = f(frame)`
(`AGENTS.md` §17, §18, §21; `STORYBOARD.md` §21, §22). No `useFrame` mutation, no
real-time clocks, no unseeded randomness. The graph's lifetime is not one
function, and this scene should not try to make it one: each scene owns a state
module over its own scene-local frames. Scene 04 writes the pose it inherits as
literals, so that its agreement with Scene 03 is a fact checked by rendering 0629
and 0630 as a pair; Scene 05 samples `getCrossRepoState(90)` instead, because a
whole graph state is twenty-odd numbers and copying them would be a second source
of truth that goes stale the first time Scene 04 is retuned. This scene follows
Scene 05: `getSemanticState` samples `getBlastState(120)`. Extending
...
Hard sequence boundary at 0840, visually continuous: at 0840 the frame equals
0839 — the accent-marked graph at rest under Scene 05's `0.72` veil, the impact
card top-left and `Exact symbols. Not name matches.` below it.

The flatten then begins. Its preconditions, owed by Scene 05:

- the graph must be **at rest** at 0840, otherwise the flatten reads as a glitch
  rather than as a change of representation;
- the accent state must carry over unchanged, so the right column is recognisably
  the same symbol;
- the card and the claim line must clear during the flatten, and both must be gone
  before 0950 — `Exact symbols. Not name matches.` and `A name is not a symbol.`
  are the same argument, and the frame must never show both.

The scene animates in scene-local frames, offsetting to absolute frames only where
it samples the graph curve shared with Scenes 03–05.

## Transition out

At 0990 `AgentAnswerScene.tsx` takes over with a **match cut** into the agent
prompt (`STORYBOARD.md` §27, *Graph → Agent*: graph contracts into selected
symbol → match cut into prompt text).

The contract with `07-agent-answer.md`:

- this scene owns the contraction, which is the two callers and their tubes
  leaving over local 139–149, and completes it at **0989**;
- at 0989 `withRetry()` is alone, settled and still, at the centre of the right
  column and at the apparent size the column drew it at;
- at 0990 Scene 07 opens on the symbol in that same place at that same size, and
  then carries it back to the prompt's token over its own duration.

Scene 07 therefore inherits **a position and an apparent size**, not the token
...
the symbol back on it — and that beat is Scene 07's to make. `07-agent-answer.md`
still describes the older contract, in which this scene delivered the node
pre-placed on the token; that sentence needs changing when someone next opens it.
...
Two consequences worth stating. The digit collision with Scene 05's
...
seven. Seven is the blast radius, and the blast radius is Scene 05's subject —
...
```text
0840 — frame identical to 0839; flatten begins
0900 — split view established, both matches highlighted, right side resolving
0950 — KEY VISUAL: name matching vs semantic resolution + "A name is not a symbol."
0989 — contraction settled, ready for the match cut
```

**Frame 0950 is a key visual frame** (`STORYBOARD.md` §29 and `AGENTS.md`'s
still-image key frame list), earmarked for technical marketing. It must work as
a still image, which means at 0950:

- the two-versus-one asymmetry is countable at a glance;
- both counters (`2 matches`, `1 symbol`, `2 real relationships`) are legible;
- `A name is not a symbol.` does not overlap or crowd either column;
...
- **They are still not benchmark figures.** `STORYBOARD.md` labels the right-hand
  ...
  `08-benchmark.md`'s `7 / 7 exact answers` counts benchmark questions answered
...
- **The right column shows the resolution, not the blast radius.** One symbol and
  the relationships that reach it. The seven affected symbols belong to Scene 05's
...
- **The copy is exact.** `A name is not a symbol.` — one sentence, that
  punctuation. One of the video's five allowed phrases (`STORYBOARD.md` §32).
- **The layout does not reflow at 0950.** The centre band is reserved from the
  start of the scene.
- **The transition into the split comes from the content.** The graph flattens out
  ...
  (`STORYBOARD.md` §27).
- **The contraction is a subtraction and it is settled at 0989.** The callers
  leave; the surviving node does not move, rescale or lose its accent. Scene 07
  inherits its position and apparent size and moves it from there.
- **Frame 0950 works as a still image.**
...
- exact frames of the left build and the right resolve, provided both are complete
  and both counters are legible by roughly 0925, and provided the left side arrives
...
- exact opacity the left side settles at, provided it stays readable at 0950;
...
- the flatten's exact pixels per world unit and column offset, provided every
  surviving label stays above the small-text floor and the reserved copy band
  stays clear;
...
- Global scene boundaries live only as inline `<Sequence>` literals in
  `src/Composition.tsx` (`from={840}`, `durationInFrames={150}`), so Remotion
  Studio can trim them. There is no timing module; do not introduce one and do not
  restate 0840/0990 inside the scene. The scene animates in scene-local frames.
...
- The left column and the counters are DOM composited over the canvas, like Scene
  05's card. The rows are data, not markup: they come from `nameMatches` in
...
- Tokens: `src/brand/tokens.ts`. Fonts: `src/brand/fonts.ts` (`"Geist"`,
  ...
  frame.
- The hand-over at 0990 is a cross-component contract, and it is a position and an
  apparent size rather than a rect. This scene leaves `withRetry()` where the
  right column drew it; Scene 07 reads that and animates from it to the prompt's
  ...
  the return closes that loop rather than repeating it. Inspect 0989 and 0990 as a
...
- Performance: the flatten animates camera plus per-node depth and opacity while a
  ...
  (`AGENTS.md` §35, §36).
- Iterate with Remotion Studio plus small range renders over roughly 0830–1000.
  Scrub forward, backward, forward across both seams (0839/0840 and 0989/0990) and
  confirm identical rendering (`AGENTS.md` §40).

## Current compromises

- **Implementation in progress.** This document is the specification the
  implementation must satisfy. `src/three/semanticState.ts` holds the scene's
  state — the flatten, the rig's straightening, the surviving subgraph, both veils
  and the column schedules — and the scene component that composes it is being
  written against this document. Everything the flatten operates on shipped with
  Scenes 03–05: `src/data/graphDemo.ts`, `src/three/graphFrame.ts`,
...
- **Ownership of the contraction is an open decision resolved here.**
  `STORYBOARD.md` SCENE 07 asks for a hold across `0970–0990`, while SCENE 08
  places "el grafo se contrae hacia el nodo seleccionado" at frame `0990` — which
  is unreachable, because 0990 is the first frame of `AgentAnswerScene` and the
  graph no longer exists there. Resolution agreed with `07-agent-answer.md`:
  `SemanticScene` owns the contraction, the hold occupies roughly `0970–0979` and
  the contraction `0979–0989`, and the storyboard's "se contrae hacia el nodo
...
- **Internal beat frames are this document's proposal.** The storyboard fixes only
  the transition, the `0930–0970` fade, the copy at `0950` and the hold. The
  flatten window, the left build and the right resolve windows in `## Timeline`
  are derived from the critical frames `0840`, `0900`, `0950` and `0989`.
