...
| Master        | Local         | Beat                                                       |
| ------------- | ------------- | ---------------------------------------------------------- |
| `1930`–`1960` | `0000`–`0030` | Integrations: `Claude Code · Codex · OpenCode · Oh My Pi`.   |
| `1960`        | `0030`        | CTA reads.                                                  |
| `1980`        | `0050`        | Install line, **only if** it stays short (see below).        |
| `1990`–`2050` | `0060`–`0120` | Full hold. Nothing animates. Roughly the final second is pure reading time. |
...
At `1930` the frame is what `08-brand.md` left at `1929`: the Kivgraph lockup and
...
At `2049` the frame holds, static:
...
`2049` is the last frame of the master and is the natural poster frame for the
...
last accent is spent at `1828`, on a relationship arriving.
...
**Integrations (`1930`–`1960`).** Opacity `0 → 1` with a small upward settle.
...
name, which is what four names fit into the window: they land at local `0320`,
`0323`, `0326` and `0329`, one frame before the URL. A stagger running past
`0330` would put a name still arriving under a URL that is supposed to be
legible.

**CTA (`1960`).** Opacity `0 → 1` with a minimal settle, concluding promptly.
Storyboard frame numbers mark when a beat **reads**, not when its ramp starts, so
the URL is legible at `1960` rather than starting to appear there.

**Install line (`1980`), if shown.** The quietest entrance in the video. Opacity
only, no travel. It must not pull attention off the URL.

**Hold (`1990`–`2050`).** Nothing animates. This is a hard requirement, not a
...
- no residual easing still resolving into `1991`.

Every element must have reached its final value by `1990`.
...
There is no transition at `1930`. The frame continues.

`08-brand.md` settles at `1440` and holds through `1929`; this scene inherits that
...
None. `2049` is the last frame of the master.
...
the cut from `2049` back to `0300` is acceptable — `STORYBOARD.md` §34 requires
...
```text
frame 1930 — inherited brand frame; integrations begin to arrive
frame 1990 — everything present and settled; the hold begins
frame 2049 — final frame of the master; poster frame candidate
```

All three are on the manual review list in `STORYBOARD.md` §28. None is a
designated still-image key frame in `AGENTS.md`, but `2049` will be used as a
poster frame in practice and should be inspected as an exported image.

Check `1991` specifically: it is the frame that proves nothing is still easing
into place after the hold begins.

## Invariants

- **Nothing animates during the hold `1990`–`2050`.** No exceptions, including
...
- The lockup and tagline do not move at the `1930` boundary.
...
- The video does not fade out. `2049` is a usable still.
...
- Render it with no props. `BrandLogo` and `BrandTagline` default every
  ...
  to `0` — so `<BrandLogo />` and `<BrandTagline />` reproduce frame `1929`
  exactly, which is what this scene must inherit. Scene 08 turns the mark once as
  its wordmark enters and that turn concludes at `1836`; nothing in this scene
  turns, and nothing here may reintroduce it.
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="09 Outro" from={1930} durationInFrames={120}>` literals, because
...
- **Nothing moves after `1960`, not `1990`.** Omitting the install line removed
  ...
  ever added back, it goes before `1960`, never inside the hold.
...
- **Lockup persistence across `1930` — resolved 2026-08-26 in favour of
