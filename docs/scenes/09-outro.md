...
action into a choice, and a choice at the end of a twenty-two-second video is not
taken at all.

The integrations line exists for one reason: the video has been talking about
coding agents for twenty seconds without saying which ones. Naming
...
lockup and its tagline, and settled completely by `1200`.
...
- Global frames: `1210`–`1330`
- Scene-local frames: `0000`–`0120` (last rendered frame `0119` / master `1329`)
- Time: 20.17 s – 22.17 s
...
| Master        | Local         | Beat                                                       |
| ------------- | ------------- | ---------------------------------------------------------- |
| `1210`–`1240` | `0000`–`0030` | Integrations: `Claude Code · Codex · OpenCode`.              |
| `1240`        | `0030`        | CTA reads.                                                  |
| `1260`        | `0050`        | Install line, **only if** it stays short (see below).        |
| `1270`–`1330` | `0060`–`0120` | Full hold. Nothing animates. Roughly the final second is pure reading time. |
...
At `1210` the frame is what `08-brand.md` left at `1209`: the Kivgraph lockup and
...
At `1329` the frame holds, static:
...
`1329` is the last frame of the master and is the natural poster frame for the
...
**Integrations (`1210`–`1240`).** Opacity `0 → 1` with a small upward settle.
...
**CTA (`1240`).** Opacity `0 → 1` with a minimal settle, concluding promptly.
Storyboard frame numbers mark when a beat **reads**, not when its ramp starts, so
the URL is legible at `1240` rather than starting to appear there.

**Install line (`1260`), if shown.** The quietest entrance in the video. Opacity
only, no travel. It must not pull attention off the URL.

**Hold (`1270`–`1330`).** Nothing animates. This is a hard requirement, not a
...
- no residual easing still resolving into `1271`.

Every element must have reached its final value by `1270`.
...
There is no transition at `1210`. The frame continues.

`08-brand.md` settles at `1200` and holds through `1209`; this scene inherits that
...
None. `1329` is the last frame of the master.
...
the cut from `1329` back to `0000` is acceptable — `STORYBOARD.md` §34 requires
...
```text
frame 1210 — inherited brand frame; integrations begin to arrive
frame 1270 — everything present and settled; the hold begins
frame 1329 — final frame of the master; poster frame candidate
```

All three are on the manual review list in `STORYBOARD.md` §28. None is a
designated still-image key frame in `AGENTS.md`, but `1329` will be used as a
poster frame in practice and should be inspected as an exported image.

Check `1271` specifically: it is the frame that proves nothing is still easing
into place after the hold begins.

## Invariants

- **Nothing animates during the hold `1270`–`1330`.** No exceptions, including
...
- The lockup and tagline do not move at the `1210` boundary.
...
- The video does not fade out. `1329` is a usable still.
...
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="09 Outro" from={1210} durationInFrames={120}>` literals, because
...
- **Open decision — lockup persistence across `1210`.** This document assumes the
