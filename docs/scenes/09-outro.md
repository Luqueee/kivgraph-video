...
- Global frames: `1530`–`1650`
- Scene-local frames: `0000`–`0120` (last rendered frame `0119` / master `1649`)
- Time: 25.5 s – 27.5 s
...
| Master        | Local         | Beat                                                       |
| ------------- | ------------- | ---------------------------------------------------------- |
| `1530`–`1560` | `0000`–`0030` | Integrations: `Claude Code · Codex · OpenCode · Oh My Pi`.   |
| `1560`        | `0030`        | CTA reads.                                                  |
| `1580`        | `0050`        | Install line, **only if** it stays short (see below).        |
| `1590`–`1650` | `0060`–`0120` | Full hold. Nothing animates. Roughly the final second is pure reading time. |
...
At `1530` the frame is what `08-brand.md` left at `1529`: the Kivgraph lockup and
...
At `1649` the frame holds, static:
...
`1649` is the last frame of the master and is the natural poster frame for the
...
last accent is spent at `1428`, on a relationship arriving.
...
**Integrations (`1530`–`1560`).** Opacity `0 → 1` with a small upward settle.
...
**CTA (`1560`).** Opacity `0 → 1` with a minimal settle, concluding promptly.
Storyboard frame numbers mark when a beat **reads**, not when its ramp starts, so
the URL is legible at `1560` rather than starting to appear there.

**Install line (`1580`), if shown.** The quietest entrance in the video. Opacity
only, no travel. It must not pull attention off the URL.

**Hold (`1590`–`1650`).** Nothing animates. This is a hard requirement, not a
...
- no residual easing still resolving into `1591`.

Every element must have reached its final value by `1590`.
...
There is no transition at `1530`. The frame continues.

`08-brand.md` settles at `1440` and holds through `1529`; this scene inherits that
...
None. `1649` is the last frame of the master.
...
the cut from `1649` back to `0000` is acceptable — `STORYBOARD.md` §34 requires
...
```text
frame 1530 — inherited brand frame; integrations begin to arrive
frame 1590 — everything present and settled; the hold begins
frame 1649 — final frame of the master; poster frame candidate
```

All three are on the manual review list in `STORYBOARD.md` §28. None is a
designated still-image key frame in `AGENTS.md`, but `1649` will be used as a
poster frame in practice and should be inspected as an exported image.

Check `1591` specifically: it is the frame that proves nothing is still easing
into place after the hold begins.

## Invariants

- **Nothing animates during the hold `1590`–`1650`.** No exceptions, including
...
- The lockup and tagline do not move at the `1530` boundary.
...
- The video does not fade out. `1649` is a usable still.
...
- Render it with no props. `BrandLogo` and `BrandTagline` default every
  ...
  to `0` — so `<BrandLogo />` and `<BrandTagline />` reproduce frame `1529`
  exactly, which is what this scene must inherit. Scene 08 turns the mark once as
  its wordmark enters and that turn concludes at `1436`; nothing in this scene
  turns, and nothing here may reintroduce it.
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="09 Outro" from={1530} durationInFrames={120}>` literals, because
...
- **The column sits 18 px below the frame's centre.** It runs `300` to `816`, so
  ...
  `1530`. Same instruction as `STORYBOARD.md` §29 gives for the `1190` still: a
...
- **Nothing moves after `1560`, not `1590`.** Omitting the install line removed
  ...
  ever added back, it goes before `1560`, never inside the hold.
...
- **Lockup persistence across `1530` — resolved 2026-08-26 in favour of
