...
- Global frames: `1630`–`1750`
- Scene-local frames: `0000`–`0120` (last rendered frame `0119` / master `1749`)
...
| Master        | Local         | Beat                                                       |
| ------------- | ------------- | ---------------------------------------------------------- |
| `1630`–`1660` | `0000`–`0030` | Integrations: `Claude Code · Codex · OpenCode · Oh My Pi`.   |
| `1660`        | `0030`        | CTA reads.                                                  |
| `1680`        | `0050`        | Install line, **only if** it stays short (see below).        |
| `1690`–`1750` | `0060`–`0120` | Full hold. Nothing animates. Roughly the final second is pure reading time. |
...
At `1630` the frame is what `08-brand.md` left at `1629`: the Kivgraph lockup and
...
At `1749` the frame holds, static:
...
`1749` is the last frame of the master and is the natural poster frame for the
...
- `Claude Code · Codex · OpenCode · Oh My Pi` in `Geist` at the UI-to-body range
  ...
  Measured at 24 px: the line runs `764` to `1314`, so it is 511 px wide — 27 %
...
last accent is spent at `1528`, on a relationship arriving.
...
**Integrations (`1630`–`1660`).** Opacity `0 → 1` with a small upward settle.
...
`0023`, `0026` and `0029`, one frame before the URL. A stagger running past
`0030` would put a name still arriving under a URL that is supposed to be
legible.

**CTA (`1660`).** Opacity `0 → 1` with a minimal settle, concluding promptly.
Storyboard frame numbers mark when a beat **reads**, not when its ramp starts, so
the URL is legible at `1660` rather than starting to appear there.

**Install line (`1680`), if shown.** The quietest entrance in the video. Opacity
only, no travel. It must not pull attention off the URL.

**Hold (`1690`–`1750`).** Nothing animates. This is a hard requirement, not a
...
- no residual easing still resolving into `1691`.

Every element must have reached its final value by `1690`.
...
There is no transition at `1630`. The frame continues.

`08-brand.md` settles at `1440` and holds through `1629`; this scene inherits that
...
None. `1749` is the last frame of the master.
...
the cut from `1749` back to `0000` is acceptable — `STORYBOARD.md` §34 requires
...
```text
frame 1630 — inherited brand frame; integrations begin to arrive
frame 1690 — everything present and settled; the hold begins
frame 1749 — final frame of the master; poster frame candidate
```

All three are on the manual review list in `STORYBOARD.md` §28. None is a
designated still-image key frame in `AGENTS.md`, but `1749` will be used as a
poster frame in practice and should be inspected as an exported image.

Check `1691` specifically: it is the frame that proves nothing is still easing
into place after the hold begins.

## Invariants

- **Nothing animates during the hold `1690`–`1750`.** No exceptions, including
...
- The lockup and tagline do not move at the `1630` boundary.
...
- The video does not fade out. `1749` is a usable still.
...
- Render it with no props. `BrandLogo` and `BrandTagline` default every
  ...
  to `0` — so `<BrandLogo />` and `<BrandTagline />` reproduce frame `1629`
  exactly, which is what this scene must inherit. Scene 08 turns the mark once as
  its wordmark enters and that turn concludes at `1536`; nothing in this scene
  turns, and nothing here may reintroduce it.
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="09 Outro" from={1630} durationInFrames={120}>` literals, because
...
- **The column sits 18 px below the frame's centre.** It runs `300` to `876`, so
  ...
  `1630`. Same instruction as `STORYBOARD.md` §29 gives for the `1290` still: a
...
- **Nothing moves after `1660`, not `1690`.** Omitting the install line removed
  ...
  ever added back, it goes before `1660`, never inside the hold.
...
- **Lockup persistence across `1630` — resolved 2026-08-26 in favour of
