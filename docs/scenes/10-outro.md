...
The name landed in `09-brand.md`. Everything left is conversion, and conversion
...
action into a choice, and a choice at the end of a twenty-three-and-a-half-second
video is not taken at all.

The integrations line exists for one reason: the video has been talking about
coding agents for twenty-one and a half seconds without saying which ones. Naming
...
Immediately before: `09-brand.md` resolved the convergence into the Kivgraph
lockup and its tagline, and settled completely by `1280`.
...
implemented as a single component. Storyboard SCENE 11 is document 10. See
`docs/scenes/README.md`.

- Global frames: `1290`–`1410`
- Scene-local frames: `0000`–`0120` (last rendered frame `0119` / master `1409`)
- Time: 21.5 s – 23.5 s
...
| Master        | Local         | Beat                                                       |
| ------------- | ------------- | ---------------------------------------------------------- |
| `1290`–`1320` | `0000`–`0030` | Integrations: `Claude Code · Codex · OpenCode`.              |
| `1320`        | `0030`        | CTA reads.                                                  |
| `1340`        | `0050`        | Install line, **only if** it stays short (see below).        |
| `1350`–`1410` | `0060`–`0120` | Full hold. Nothing animates. Roughly the final second is pure reading time. |
...
At `1290` the frame is what `09-brand.md` left at `1289`: the Kivgraph lockup and
...
At `1409` the frame holds, static:
...
`1409` is the last frame of the master and is the natural poster frame for the
...
is the lockup's mark, inherited from `09-brand.md`. The URL is **not** accented.
...
**Integrations (`1290`–`1320`).** Opacity `0 → 1` with a small upward settle.
...
**CTA (`1320`).** Opacity `0 → 1` with a minimal settle, concluding promptly.
Storyboard frame numbers mark when a beat **reads**, not when its ramp starts, so
the URL is legible at `1320` rather than starting to appear there.

**Install line (`1340`), if shown.** The quietest entrance in the video. Opacity
only, no travel. It must not pull attention off the URL.

**Hold (`1350`–`1410`).** Nothing animates. This is a hard requirement, not a
...
- no residual easing still resolving into `1351`.

Every element must have reached its final value by `1350`.
...
There is no transition at `1290`. The frame continues.

`09-brand.md` settles at `1280` and holds through `1289`; this scene inherits that
...
This depends on the derived continuity decision recorded in `09-brand.md`
...
the frame, and `09-brand.md` must be authored to that position from the start.

## Transition out

None. `1409` is the last frame of the master.
...
the cut from `1409` back to `0000` is acceptable — `STORYBOARD.md` §34 requires
...
```text
frame 1290 — inherited brand frame; integrations begin to arrive
frame 1350 — everything present and settled; the hold begins
frame 1409 — final frame of the master; poster frame candidate
```

All three are on the manual review list in `STORYBOARD.md` §28. None is a
designated still-image key frame in `AGENTS.md`, but `1409` will be used as a
poster frame in practice and should be inspected as an exported image.

Check `1351` specifically: it is the frame that proves nothing is still easing
into place after the hold begins.

## Invariants

- **Nothing animates during the hold `1350`–`1410`.** No exceptions, including
...
- The lockup and tagline do not move at the `1290` boundary.
...
- The video does not fade out. `1409` is a usable still.
...
- Global scene boundaries live inline in `src/Composition.tsx` as
  `<Sequence name="10 Outro" from={1290} durationInFrames={120}>` literals, because
...
- **Open decision — lockup persistence across `1290`.** This document assumes the
  lockup and tagline carry over from `09-brand.md` unchanged. The storyboard
  neither states nor forbids it. Mirrored in `09-brand.md`
