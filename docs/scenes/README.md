...
| 00  | Intent              |   120–370 | `IntentScene.tsx`      | [00-intent.md](./00-intent.md)                            |
| 01  | Symbol              |   370–490 | `SymbolScene.tsx`      | [01-symbol.md](./01-symbol.md)                            |
| 02  | Agent               |   490–700 | `AgentScene.tsx`       | [02-agent.md](./02-agent.md)                              |
| 03  | Graph Reveal        |  700–1060 | `GraphRevealScene.tsx` | [03-graph-reveal.md](./03-graph-reveal.md)                |
| 04  | Blast Radius        | 1060–1200 | `BlastRadiusScene.tsx` | [04-blast-radius.md](./04-blast-radius.md)                |
| 05  | Semantic Resolution | 1200–1400 | `SemanticScene.tsx`    | [05-semantic-resolution.md](./05-semantic-resolution.md)  |
| 06  | Agent Answer        | 1400–1580 | `AgentAnswerScene.tsx` | [06-agent-answer.md](./06-agent-answer.md)                |
| 07  | Benchmark           | 1580–1830 | `BenchmarkScene.tsx`   | [07-benchmark.md](./07-benchmark.md)                      |
| 08  | Brand               | 1830–2000 | `BrandScene.tsx`       | [08-brand.md](./08-brand.md)                              |
| 09  | Outro               | 2000–2120 | `OutroScene.tsx`       | [09-outro.md](./09-outro.md)                              |
...
Master: 1920 × 1080, 60 fps, **2120 frames, 35.33 s**. Global frame boundaries
...
numbers met, and `src/Composition.tsx` now exports `masterFrames = 2120`.

Rendered end to end: 2120 frames, 35.33 s, 1920 × 1080 at 60 fps, and no black
frame anywhere in it.

The master length has moved twelve times. Three were because the opening changed:
it was 1450, grew to 1970 when scene 01 was extended from 90 to 210 frames,
returned to 1450 when the old scene 02 was deleted, and fell to 1760 when scene
01 was cut from 210 to 120. The fourth was the first one a later scene caused —
1760 to 1670, when the cross-repository scene was cut. The fifth, on 2026-08-25,
is the first one a *surviving* scene caused: 1670 to 1650, when the blast radius
...
is the first one that added time rather than removing it: 1650 to 1680, when the
...
and the first taken against a measurement rather than a judgement: 1680 to
**1830**, when the blast radius went 100 → 140, the semantic resolution 180 → 200
...
the first taken *before* a scene shipped rather than after: 1830 to **1880**, when
...
time any scene has changed duration twice at implementation: 1880 to **1920**,
...
than by counting: 1920 to **2000**, when the brand reveal went from the 90 frames
...
The twelfth, the same day, is the only one that has ever given time back: 2170 to
**2120**, when scene 00 lost the couplet it opened on and went 300 → 250. The
fifty frames were returned to the film rather than spent inside the scene — every
window in it moved by that constant, so nothing in the candidate stack or the
match cut arrives faster than it did. Seven scenes have now changed duration, and
scene 00 is the first to shrink.

The eleventh, on 2026-08-27, is the only one that did not come from dwell inside
an existing scene: 2000 to **2120**, when the cold open was added at the front so
...
Measured on the 1730-frame cut, two elements failed badly:
...
then stands byte-identical for 67 frames from 1752 before the fade.
...
- the opening is **one continuous shot** through **one world**. There is no cut
  anywhere before frame 0700. `src/world/camera.ts` projects it and
...
- the camera is a **rig** — an eye and a point it looks at — not a position with
  ...
  move completes at 1028 and no frame after it explores, though two later scenes
...
holds from 1028 onward — the `lookAt` rig at eye `(7.0, 3.2, 10.0)` looking at
...
JetBrains Mono at 480; the deficit was the blend model — troika's SDF coverage is
linear alpha, Chrome's text is gamma-corrected — and it was invariant to
`sdfGlyphSize` (`-20.83%` at 64, `-20.88%` at 336), which is what proved it was
...
match-cut token rectangle, at master 0730 — the frame where the dissolve's
...
splits the graph into `SCENE 03 — FROM CODE TO GRAPH` (0700–0770) and
`SCENE 04 — THE GRAPH EXPANDS` (0770–1060), both realised by the single
...
CROSS-REPOSITORY`, 90 frames at master 0940–0950 under the numbering of the time.
...
at 0924–0999, and the propagation still leaves one repository and arrives in
...
settled graph, evenly present, via `getGraphState(380)` where it used to sample
`getCrossRepoState(90)`, and its propagation reads as travel rather than as the
re-lighting of something held back for it.

Everything after 0940 moved 90 frames earlier and the master went from 1760 to
1790. The camera pose the deleted scene landed on was scene 03's own returned to,
so nothing about the graph's geometry changed with it: the only rig moves after
1028 are scene 05 straightening into its flatten and scene 06's twenty-four-frame
...
again once scene 08 landed: the 0700 and 1200 seams are pixel-identical, 1060 is
62.93 dB (glyph antialiasing only), and 2120 mounted frames render with no black
frame.

Re-measured after the cold open landed and again after scene 00 was cut to 250
frames, on PNG stills, at 2120 frames:

```text
0119/0128   inf        the ten empty frames the teaser hands to the intent scene
0369/0370   45.01 dB   candidate -> source, unchanged by either retime
1059/1060   66.36 dB   glyph antialiasing only
1579/1580   22.21 dB   the hard cut into the benchmark
1829/1830   inf        fade -> silence
1999/2000   inf        the lockup crossing into the outro
```
...
pure `#000000` at 1826 and held it to 1829, and scene 08 then restored `#0a0b0d`
at 1830. A ten-level step on a flat frame, at the one boundary in the film that
is meant to be invisible, and precisely the levels change `08-brand.md` forbids
by name. The fade now lives on an inner fill; 1658, 1770 and 1817 are
byte-identical to the render before it, because `fadeOut` is 1 for every frame
before local 198.

The 1829/1830 seam therefore measures `inf` — the two frames are identical, and
the corner holds `10 11 13` across it. Scene 07 does not render the silence; it
lands on it two frames early, at 1828, because the project's easing is within one
335th of its final value before its ramp ends, and scene 08's own ten frames join
it into one twelve-frame identical run.

The 1400 boundary does not measure like those, and it is not supposed to. It is a
match cut, not an invisible cut: the symbol region across 1399/1400 measures
...
whole-film scan flags two pairs and nothing else: 1399/1400 and
1579/1580. Scene 08 adds no third: its own boundary at 1830 is identical rather
than a cut, and it contains no cut of its own. Both are hard cuts, steps rather than spikes, confirmed by the frames
either side, and 1579/1580 is scene 07's own cut at 24.25 dB, which is what a hard
...
calling it a regression: at 1579/1580 the attribution region measures 55.97 dB
...
frames 0460–0610 under the numbering of the time.
...
- **A stale number shifted is still stale, and now differently.** Half the work
  ...
  `from={}` offsets three retimes old, and the still-image key frame was `1540` in
  one document and `1538` in another. Shifting any of those would have produced a
...
Frames that must hold up as still images: `0058`, `0450`, `1059`, `1148`, `1294`,
`1494`, `1658`, `1910`. Each is documented in the scene that owns it, and each is
...
| `0450` | scene 01's arrival: everything has landed together, first settled frame. |
| `1059` | scene 03's last frame — the settled graph, the image scene 04 inherits.  |
| `1148` | the frame scene 04's impact card settles on; first frame of its read.    |
| `1294` | the first frame of scene 05's measured byte-identical stand.             |
| `1494` | the frame scene 06's attribution label finishes on; the scene goes static.|
| `1658` | scene 07's cost row complete, with both arms already named above it.     |
| `1910` | scene 08's settled lockup plus tagline; static since `1908`.             |

How they got here. `1494` joined the list as scene 06's label frame — it was
`1070` when the scene landed and `1400` after the semantic scene grew. Of the six
that predate it, the four after `1059` each moved −90 with the cross-repository
...
them: `1140` → `1148`, `0970` → `1294`, `1400` → `1494`, `1470` → `1658` and
`1270` → `1820`. Only `0450` and `1059` are untouched by it. The 2026-08-26
benchmark pass moved one of them: `1820` → `1570`, carried by scene 08's +50
offset, and the same day's rebuild of scene 07 moved it again, `1570` → `1910`,
carried by scene 08's further +40. `1658` has not moved through either pass,
...
written down. `1148` is +8 rather than +40, because the card's entry window moved
inside its own scene as well as the scene moving. `1294` is +24 rather than +40,
...
Scene 03's entry moved from `0970` to `1059`. Both frames sit inside the held
final camera pose, but at 0970 the last crossing is still handing its accent back
to the structure; `1059` is the last frame the scene owns, the nearest thing to
settled, and the image `04-blast-radius.md` inherits one frame later — so the still
and the handoff are the same picture. `STORYBOARD.md` §28-§29 still list `0970`
and have not been reconciled.

`1294` lost its headline rather than just its number. It used to hold
...
`1494` is scene 06's label frame, and it sits at the head of a hold rather than on
the last frame that moves: measured on the render, the scene stops changing at 1493
and every frame from there to 1579 is byte-identical, so the still is one frame
...
whole film by at most 15/335 on ~2.4% of pixels — visually indistinguishable,
...
```text
2026-08-27
- Eleventh master length, and two scenes grew for two different reasons: 1650 ->
  1750 frames, 29.17 s. Scene 03 300 -> 360, scene 07 210 -> 250.
- Scene 03, so the cross-repository reach is a beat. The three crossings used to
  start while the camera was already opening, so the new space and the edges that
  reach it arrived together and neither read. Now the local graph completes, its
  label settles, nothing happens for eighteen frames, one crossing draws alone for
  forty-four with the rig parked - leaving the frame toward space the viewer
  cannot see - and only then does the camera follow it. The other two crossings
  are confirmation.
- Scene 07, for the line that joins it to the answer: `Same answer. Less context.`
  It reads at local 30 and is gone by 62 as the column heads land, so it never
  shares a frame with the table, and the scene opens as a relay of three single
  things. New copy, so STORYBOARD.md §32 took it in the same task. The table's
  dwell is untouched - the 62-frame settled hold is still 62.
- Key frames recomputed from their own structural definitions rather than
  dragged: 0629 -> 0689, the scene's last frame with the graph settled, and 1190
  -> 1288, the first frame with the cost row complete and both arms named.
- Every frame reference in this repository's documents was shifted in one pass.
  The shift is piecewise, not uniform - scene 07 grew and also moved its table
  40 frames later inside itself, so its own frames move +60 or +100 depending on
  whether they belong to the inherited attribution or to the table.
- Measured: 1750 frames, 29.17 s, no black frame anywhere.
```

```text
2026-08-27
- New scene 00, Intent, and the film's third match cut. 1750 -> 1930 frames,
  32.17 s. Nothing else was redesigned: every scene from 01 on moved by a
  constant 180 and nothing inside any of them changed.
- Why: the film opened on `withRetry` already singled out, which quietly claimed
  the agent knew what the symbol was called. `find_by_intent` is the tool for the
  step before that, and without it the story said Kivgraph is useful once you
  already know where to look.
- The semantics come from the tool's own documentation, not from a description of
  it. `match` travels on every row - `lexical`, `lexical+calls` - because every
  row of the real answer carries it; no score is shown, because the tool
  publishes none and says why; the candidates are derived from graphDemo nodes
  rather than typed, under the rule that file already carries about fabricated
  fixtures. The intent line is the documented example verbatim.
- Candidate -> source is measured, not assumed: candidate ink at 0179 and scene
  01's at 0180 both occupy x 382..855, y 626..711 against 626..710. Zero error on
  three edges, one pixel of antialiasing on the descender. `symbolAnchor` and
  `symbolOpeningZoom` are now exported from SymbolScene so the two scenes cannot
  derive the point differently.
- Source -> Three.js is untouched and re-measured at its new boundary, 0509/0510:
  PSNR inf whole-frame and over the token region.
- Scene 01's 39-frame hold was tested for removal and kept, on a measurement. Its
  caption is 36 characters settling at local 0080, so the hold is 0.65 s - 55
  characters per second against the 25-40 budget. It is already under the
  minimum; trimming it makes the caption unreadable rather than the film tighter.
- Two documentation defects surfaced while retiming. Beat tables have a master and
  a local column and a blanket shift moves both, which corrupts the local one; and
  ranges written as a single backticked span were invisible to the earlier
  retimes, so scene 04's master column had been stale since the +60 before this.
  Both are fixed by deriving every master cell from its own local cell plus the
  scene start, which is self-correcting and is how it should have been done.
```

```text
2026-08-27
- Scene 00 restructured into three beats - problem, tool, result - and grown 180
  -> 220 frames. Master 1930 -> 1970, 32.83 s. Everything from 0180 on moved +40;
  nothing inside any other scene changed.
- The growth is measured rather than felt. At 180 the reading budget of 25-40
  characters per second failed on three of five blocks, the worst of them
  settling two frames before the scene began to leave. Structure fixed *hard to
  parse*; it could not also fix *not on screen long enough*.
- Two documentation defects of the same family as last time, both caught by
  reading the result rather than trusting the script: numbers without a leading
  zero escape the frame-shift rule, and a range written as one backticked span
  escapes it too. Scene 01's own global range and scene 04's row in both index
  tables were wrong until they were checked.
```

```text
2026-08-27
- Scene 00 grown 220 -> 300 frames, on direct art direction to be realistic about
  reading time. Master 1970 -> 2050, 34.17 s. Everything from 0220 on moved +80;
  nothing inside any other scene changed.
- The intent line is quoted and introduced by `So you describe it:`. It had been
  prefixed with the prompt glyph, which promised a question the line is not, and
  then with nothing, which was worse. The documented phrase itself is unchanged.
- The frame-shift script now handles all four notations in one pass - backticked
  single, backticked compound, bare padded and bare unpadded - because the last
  two retimes each missed a different one of them and left a stale range behind.
```
