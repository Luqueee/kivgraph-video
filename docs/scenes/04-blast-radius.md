...
**Immediately before.** Scene 03 (`03-graph-reveal.md`, frames 0630–0990) finished
building the graph and settled: eight nodes and seven edges present, the cascade
laid out in depth, the three crossings drawn at 0854–0929, `withRetry()` accent-lit
...
| `1078`      | `0088`      | Card settled, propagation all but complete. **Key visual frame.**                          |
...
At master frame 0990 (local 0300) the frame is **visually identical** to master
frame 0989, inheriting Scene 03's camera and graph exactly:
...
- The settled cascade in depth: `withRetry()` nearest the camera at `z 0.0`,
  ...
  `-Z`. Scene 03's camera move completes at 0958 and nothing in the film moves the
...
At master frame 1130 (local 0440, the boundary frame handed to
`SemanticScene.tsx`):
...
Nothing is mid-animation at 1130, and nothing has been for 49 frames. The next
...
**Hierarchy** at 1078, strongest first:
...
**The pulse.** `withRetry()` pulses exactly **once** at 0990 — a single short
...
key frame 1078 has to hold the count and the evidence in one image. Darkening the
graph there would have taken away the reason to keep the receipt.

**Camera.** Static. Scene 03 leaves the rig at rest at 0990, so this scene holds
...
**Holds.** 1078 must be still enough to export as an image. 1082–1130 is a true
...
It first ran 1070–1110 on a 120-frame cut, and the twenty frames that came off it
...
The card now settles at 1078 and stands 51 frames of this scene, then keeps fading
...
which is what makes the far group legible at 1078.
...
Hard sequence boundary at 0990, and it is **visually continuous rather than
byte-identical**: frames 0989 and 0990 measure **62.93 dB** PSNR, and the
difference is glyph antialiasing and nothing else. Scene 03's camera move
completes at 0958 and nothing in the film moves the camera after it, so the pose
...
At 1130 `SemanticScene.tsx` takes over and the transition comes from the content
...
- the graph must be **at rest** at 1130 — a flatten applied to a still frame reads
...
```text
0990 — single pulse on withRetry(); frame otherwise identical to 0989
1040 — propagation mid-flight, ordering clearly readable
1078 — KEY VISUAL: full blast radius plus impact card, first settled frame
1129 — last frame of the scene; identical to 1082, card and propagation, nothing dimmed
```

**Frame 1078 is a key visual frame** (`STORYBOARD.md` §29, and `AGENTS.md`'s list
of frames that must remain intentional as stills: 0380, 0989, 1078, 1224, 1424,
1590, 1750), earmarked for the README. 1078 is *defined* as the frame the impact
...
- the propagation reads complete and every affected node is unambiguously marked.
  ...
  four frames after the frame stops measurably changing at 1082. The easing is
  front-loaded (`Easing.bezier(0.22, 1, 0.36, 1)`), so at 1078 the last mark is
  already deep in its own tail; if a render ever shows it short, 1082 is its
...
- **One pulse, not a loop.** `withRetry()` pulses exactly once at 0990. No
...
- **Frame 1078 works as a still image.**
- **The card's dwell is not spent.** It settles at 1078, stands 51 frames of this
...
- exact frames of the card's entry, provided it begins no earlier than the
  propagation being visibly under way, is fully settled before 1078, and leaves
...
- exact per-step propagation durations and the small gaps between steps, as long
  as the order stays unambiguous and the propagation completes by 1086, before the
  card is fully read;
...
- Global scene boundaries live only as inline `<Sequence>` literals in
  `src/Composition.tsx` (`from={710}`, `durationInFrames={140}`), so Remotion
...
- The card belongs in `src/components/MetricCard.tsx` (`AGENTS.md` §4, §31) so
  ...
  block at 1130 and fades it out during the flatten, so the two scenes have to
  render it identically; the first version had this scene own the markup, Scene 05
  simply not draw it, and the 1129/1130 seam measured 22 dB PSNR. It is now
  pixel-identical.
...
- Iterate with Remotion Studio plus a small range render over roughly 0900–1140;
  scrub forward, backward, forward and confirm 1078 renders identically each time
  (`AGENTS.md` §40).
...
- **Card entry frames are not specified by the storyboard.** It gives the motion
  ...
  1056–1078 window in `## Timeline` is this document's proposal, constrained from
  the front by the propagation being visibly under way and from the back by the
  measurement in `## Motion` → **Holds**: the card has to settle at key frame 1078
