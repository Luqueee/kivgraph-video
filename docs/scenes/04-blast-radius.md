...
**Immediately before.** Scene 03 (`03-graph-reveal.md`, frames 0700–1060) finished
building the graph and settled: eight nodes and seven edges present, the cascade
laid out in depth, the three crossings drawn at 0924–0999, `withRetry()` accent-lit
at its head, and the camera at rest on the pose it arrived at. There used to be a
scene between the two — `CrossRepoScene`, frames 0940–0950 — and it was cut; see
...
- Storyboard coverage: **`STORYBOARD.md` SCENE 06 — BLAST RADIUS**.
  ...
  whole mapping.
- Global frames: **1060–1200**
- Scene-local frames: **0000–0140**
- Time: **17.67 s – 20.0 s**
- Duration: **140 frames / 2.33 s** at 60 fps
- Remotion component: `src/scenes/BlastRadiusScene.tsx`
- Boundary source: the inline `<Sequence name="04 Blast Radius" from={1060} durationInFrames={140}>`
  literal in `src/Composition.tsx`

Beats (scene-local = master − 710):

| Master      | Local       | Beat                                                                                     |
| ----------- | ----------- | ---------------------------------------------------------------------------------------- |
| `0940–0960` | `0000–0020` | `withRetry()` pulses **once**: seven frames up, thirteen down, back to zero.              |
| `0948–0976` | `0008–0036` | Hop 1. The relation lifts at 0308, `Policy.Do()` and `Once()` are marked from 0314.       |
| `0972–1002` | `0032–0062` | Hop 2, the public bridge: `Client.Charge()` and `Client.Refund()`.                        |
| `0998–1036` | `0058–0096` | Hop 3, the three `checkout-service` consumers, four frames apart.                         |
| `1006–1028` | `0066–0088` | The impact card: `opacity 0 → 1`, `x +24 → 0`, no overshoot.                              |
| `1148`      | `0088`      | Card settled, propagation all but complete. **Key visual frame.**                          |
| `1032–1080` | `0092–0140` | The read. 49 frames measured identical; nothing moves, the frame exists to be read.       |
...
At master frame 1060 (local 0000) the frame is **visually identical** to master
frame 1059, inheriting Scene 03's camera and graph exactly:
...
- The settled cascade in depth: `withRetry()` nearest the camera at `z 0.0`,
  ...
  `-Z`. Scene 03's camera move completes at 1028 and nothing in the film moves the
...
At master frame 1200 (local 0140, the boundary frame handed to
`SemanticScene.tsx`):
...
Nothing is mid-animation at 1200, and nothing has been for 49 frames. The next
...
(`STORYBOARD.md` §7), one per line, numbers aligned. Weights 480/580 only.
...
**Hierarchy** at 1148, strongest first:
...
**The pulse.** `withRetry()` pulses exactly **once** at 1060 — a single short
...
key frame 1148 has to hold the count and the evidence in one image. Darkening the
graph there would have taken away the reason to keep the receipt.

**Camera.** Static. Scene 03 leaves the rig at rest at 1060, so this scene holds
...
**Holds.** 1148 must be still enough to export as an image. 1152–1200 is a true
...
It first ran at what was then 1070–1110, on a 120-frame cut, and the twenty frames that came off it
...
The card now settles at 1148 and stands 51 frames of this scene, then keeps fading
...
which is what makes the far group legible at 1148.
...
`STORYBOARD.md` §21, §22). The inherited layout comes from `getGraphState(380)` in
...
Hard sequence boundary at 1060, and it is **visually continuous rather than
byte-identical**: frames 1059 and 1060 measure **62.93 dB** PSNR, and the
difference is glyph antialiasing and nothing else. Scene 03's camera move
completes at 1028 and nothing in the film moves the camera after it, so the pose
...
samples `getGraphState(380)`, and because every ramp in that curve clamps, 380 and
...
At 1200 `SemanticScene.tsx` takes over and the transition comes from the content
...
- the graph must be **at rest** at 1200 — a flatten applied to a still frame reads
...
```text
1060 — single pulse on withRetry(); frame otherwise identical to 1059
1110 — propagation mid-flight, ordering clearly readable
1148 — KEY VISUAL: full blast radius plus impact card, first settled frame
1199 — last frame of the scene; identical to 1152, card and propagation, nothing dimmed
```

**Frame 1148 is a key visual frame** (`STORYBOARD.md` §29, and `AGENTS.md`'s list
of frames that must remain intentional as stills: 0450, 1059, 1148, 1294, 1494,
1658, 1910), earmarked for the README. 1148 is *defined* as the frame the impact
...
- the propagation reads complete and every affected node is unambiguously marked.
  ...
  four frames after the frame stops measurably changing at 1152. The easing is
  front-loaded (`Easing.bezier(0.22, 1, 0.36, 1)`), so at 1148 the last mark is
  already deep in its own tail; if a render ever shows it short, 1152 is its
...
- **One pulse, not a loop.** `withRetry()` pulses exactly once at 1060. No
...
- **Frame 1148 works as a still image.**
- **The card's dwell is not spent.** It settles at 1148, stands 51 frames of this
...
- exact frames of the card's entry, provided it begins no earlier than the
  propagation being visibly under way, is fully settled before 1148, and leaves
...
- exact per-step propagation durations and the small gaps between steps, as long
  as the order stays unambiguous and the propagation completes by 1156, before the
  card is fully read;
...
- Global scene boundaries live only as inline `<Sequence>` literals in
  `src/Composition.tsx` (`from={1060}`, `durationInFrames={140}`), so Remotion
  Studio can trim them. There is no timing module; do not introduce one and do not
  restate 710/850 inside the scene. The scene animates in scene-local frames, and
...
- The card belongs in `src/components/MetricCard.tsx` (`AGENTS.md` §4, §31) so
  ...
  block at 1200 and fades it out during the flatten, so the two scenes have to
  render it identically; the first version had this scene own the markup, Scene 05
  simply not draw it, and the 1199/1200 seam measured 22 dB PSNR. It is now
  pixel-identical.
...
- Iterate with Remotion Studio plus a small range render over roughly 0970–1210;
  scrub forward, backward, forward and confirm 1148 renders identically each time
  (`AGENTS.md` §40).
...
- **Card entry frames are not specified by the storyboard.** It gives the motion
  ...
  1126–1148 window in `## Timeline` is this document's proposal, constrained from
  the front by the propagation being visibly under way and from the back by the
  measurement in `## Motion` → **Holds**: the card has to settle at key frame 1148
