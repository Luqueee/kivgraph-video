...
**Immediately before.** Scene 03 (`03-graph-reveal.md`, frames 0750–1110) finished
building the graph and settled: eight nodes and seven edges present, the cascade
laid out in depth, the three crossings drawn at 0974–1049, `withRetry()` accent-lit
at its head, and the camera at rest on the pose it arrived at. There used to be a
scene between the two — `CrossRepoScene`, frames 0990–1000 — and it was cut; see
...
| Master      | Local       | Beat                                                                                     |
| ----------- | ----------- | ---------------------------------------------------------------------------------------- |
| `0990–1010` | `0000–0020` | `withRetry()` pulses **once**: seven frames up, thirteen down, back to zero.              |
| `0998–1026` | `0008–0036` | Hop 1. The relation lifts at 0308, `Policy.Do()` and `Once()` are marked from 0314.       |
| `1022–1052` | `0032–0062` | Hop 2, the public bridge: `Client.Charge()` and `Client.Refund()`.                        |
| `1048–1086` | `0058–0096` | Hop 3, the three `checkout-service` consumers, four frames apart.                         |
| `1056–1078` | `0066–0088` | The impact card: `opacity 0 → 1`, `x +24 → 0`, no overshoot.                              |
| `1198`      | `0088`      | Card settled, propagation all but complete. **Key visual frame.**                          |
| `1082–1130` | `0092–0140` | The read. 49 frames measured identical; nothing moves, the frame exists to be read.       |
...
At master frame 1110 (local 0000) the frame is **visually identical** to master
frame 1109, inheriting Scene 03's camera and graph exactly:
...
- The settled cascade in depth: `withRetry()` nearest the camera at `z 0.0`,
  ...
  `-Z`. Scene 03's camera move completes at 1078 and nothing in the film moves the
...
At master frame 1250 (local 0140, the boundary frame handed to
`SemanticScene.tsx`):
...
Nothing is mid-animation at 1250, and nothing has been for 49 frames. The next
...
**Hierarchy** at 1198, strongest first:
...
**The pulse.** `withRetry()` pulses exactly **once** at 1110 — a single short
...
key frame 1198 has to hold the count and the evidence in one image. Darkening the
graph there would have taken away the reason to keep the receipt.

**Camera.** Static. Scene 03 leaves the rig at rest at 1110, so this scene holds
...
**Holds.** 1198 must be still enough to export as an image. 1202–1250 is a true
...
The card now settles at 1198 and stands 51 frames of this scene, then keeps fading
...
which is what makes the far group legible at 1198.
...
Hard sequence boundary at 1110, and it is **visually continuous rather than
byte-identical**: frames 1109 and 1110 measure **62.93 dB** PSNR, and the
difference is glyph antialiasing and nothing else. Scene 03's camera move
completes at 1078 and nothing in the film moves the camera after it, so the pose
...
At 1250 `SemanticScene.tsx` takes over and the transition comes from the content
...
- the graph must be **at rest** at 1250 — a flatten applied to a still frame reads
...
```text
1110 — single pulse on withRetry(); frame otherwise identical to 1109
1160 — propagation mid-flight, ordering clearly readable
1198 — KEY VISUAL: full blast radius plus impact card, first settled frame
1249 — last frame of the scene; identical to 1202, card and propagation, nothing dimmed
```

**Frame 1198 is a key visual frame** (`STORYBOARD.md` §29, and `AGENTS.md`'s list
of frames that must remain intentional as stills: 0500, 1109, 1198, 1344, 1544,
1708, 1960), earmarked for the README. 1198 is *defined* as the frame the impact
...
- the propagation reads complete and every affected node is unambiguously marked.
  ...
  four frames after the frame stops measurably changing at 1202. The easing is
  front-loaded (`Easing.bezier(0.22, 1, 0.36, 1)`), so at 1198 the last mark is
  already deep in its own tail; if a render ever shows it short, 1202 is its
...
- **One pulse, not a loop.** `withRetry()` pulses exactly once at 1110. No
...
- **Frame 1198 works as a still image.**
- **The card's dwell is not spent.** It settles at 1198, stands 51 frames of this
...
- exact frames of the card's entry, provided it begins no earlier than the
  propagation being visibly under way, is fully settled before 1198, and leaves
...
- exact per-step propagation durations and the small gaps between steps, as long
  as the order stays unambiguous and the propagation completes by 1206, before the
  card is fully read;
...
- Global scene boundaries live only as inline `<Sequence>` literals in
  `src/Composition.tsx` (`from={1110}`, `durationInFrames={140}`), so Remotion
...
- The card belongs in `src/components/MetricCard.tsx` (`AGENTS.md` §4, §31) so
  ...
  block at 1250 and fades it out during the flatten, so the two scenes have to
  render it identically; the first version had this scene own the markup, Scene 05
  simply not draw it, and the 1249/1250 seam measured 22 dB PSNR. It is now
  pixel-identical.
...
- Iterate with Remotion Studio plus a small range render over roughly 1020–1260;
  scrub forward, backward, forward and confirm 1198 renders identically each time
  (`AGENTS.md` §40).
...
- **Card entry frames are not specified by the storyboard.** It gives the motion
  ...
  1176–1198 window in `## Timeline` is this document's proposal, constrained from
  the front by the propagation being visibly under way and from the back by the
  measurement in `## Motion` → **Holds**: the card has to settle at key frame 1198
