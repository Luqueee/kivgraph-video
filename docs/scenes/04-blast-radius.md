...
**Immediately before.** Scene 03 (`03-graph-reveal.md`, frames 0550–0910) finished
building the graph and settled: eight nodes and seven edges present, the cascade
laid out in depth, the three crossings drawn at 0774–0849, `withRetry()` accent-lit
at its head, and the camera at rest on the pose it arrived at. There used to be a
scene between the two — `CrossRepoScene`, frames 0910–1000 — and it was cut; see
...
- Storyboard coverage: **`STORYBOARD.md` SCENE 06 — BLAST RADIUS**.
  ...
  whole mapping.
- Global frames: **0910–1050**
- Scene-local frames: **0220–0360**
- Time: **11.5 s – 13.83 s**
...
| Master      | Local       | Beat                                                                                     |
| ----------- | ----------- | ---------------------------------------------------------------------------------------- |
| `0950–0970` | `0000–0020` | `withRetry()` pulses **once**: seven frames up, thirteen down, back to zero.              |
| `0958–0986` | `0008–0036` | Hop 1. The relation lifts at 0228, `Policy.Do()` and `Once()` are marked from 0234.       |
| `0982–1012` | `0032–0062` | Hop 2, the public bridge: `Client.Charge()` and `Client.Refund()`.                        |
| `1008–1046` | `0058–0096` | Hop 3, the three `checkout-service` consumers, four frames apart.                         |
| `1016–1038` | `0066–0088` | The impact card: `opacity 0 → 1`, `x +24 → 0`, no overshoot.                              |
| `0998`      | `0088`      | Card settled, propagation all but complete. **Key visual frame.**                          |
| `1042–1090` | `0092–0140` | The read. 49 frames measured identical; nothing moves, the frame exists to be read.       |
...
At master frame 0910 (local 0220) the frame is **visually identical** to master
frame 0909, inheriting Scene 03's camera and graph exactly:
...
- The settled cascade in depth: `withRetry()` nearest the camera at `z 0.0`,
  ...
  `-Z`. Scene 03's camera move completes at 0878 and nothing in the film moves the
...
At master frame 1050 (local 0360, the boundary frame handed to
`SemanticScene.tsx`):
...
Nothing is mid-animation at 1050, and nothing has been for 49 frames. The next
...
**Hierarchy** at 0998, strongest first:
...
**The pulse.** `withRetry()` pulses exactly **once** at 0910 — a single short
...
key frame 0998 has to hold the count and the evidence in one image. Darkening the
graph there would have taken away the reason to keep the receipt.

**Camera.** Static. Scene 03 leaves the rig at rest at 0910, so this scene holds
...
**Holds.** 0998 must be still enough to export as an image. 1002–1050 is a true
...
It first ran 0990–1030 on a 120-frame cut, and the twenty frames that came off it
...
The card now settles at 0998 and stands 51 frames of this scene, then keeps fading
...
which is what makes the far group legible at 0998.
...
Hard sequence boundary at 0910, and it is **visually continuous rather than
byte-identical**: frames 0909 and 0910 measure **62.93 dB** PSNR, and the
difference is glyph antialiasing and nothing else. Scene 03's camera move
completes at 0878 and nothing in the film moves the camera after it, so the pose
...
At 1050 `SemanticScene.tsx` takes over and the transition comes from the content
...
- the graph must be **at rest** at 1050 — a flatten applied to a still frame reads
...
```text
0910 — single pulse on withRetry(); frame otherwise identical to 0909
0960 — propagation mid-flight, ordering clearly readable
0998 — KEY VISUAL: full blast radius plus impact card, first settled frame
1049 — last frame of the scene; identical to 1002, card and propagation, nothing dimmed
```

**Frame 0998 is a key visual frame** (`STORYBOARD.md` §29, and `AGENTS.md`'s list
of frames that must remain intentional as stills: 0300, 0909, 0998, 1144, 1344,
1510, 1670), earmarked for the README. 0998 is *defined* as the frame the impact
...
- the propagation reads complete and every affected node is unambiguously marked.
  ...
  four frames after the frame stops measurably changing at 1002. The easing is
  front-loaded (`Easing.bezier(0.22, 1, 0.36, 1)`), so at 0998 the last mark is
  already deep in its own tail; if a render ever shows it short, 1002 is its
...
- **One pulse, not a loop.** `withRetry()` pulses exactly once at 0910. No
...
- **Frame 0998 works as a still image.**
- **The card's dwell is not spent.** It settles at 0998, stands 51 frames of this
...
- exact frames of the card's entry, provided it begins no earlier than the
  propagation being visibly under way, is fully settled before 0998, and leaves
...
- exact per-step propagation durations and the small gaps between steps, as long
  as the order stays unambiguous and the propagation completes by 1006, before the
  card is fully read;
...
- The card belongs in `src/components/MetricCard.tsx` (`AGENTS.md` §4, §31) so
  ...
  block at 1050 and fades it out during the flatten, so the two scenes have to
  render it identically; the first version had this scene own the markup, Scene 05
  simply not draw it, and the 1049/1050 seam measured 22 dB PSNR. It is now
  pixel-identical.
...
- Iterate with Remotion Studio plus a small range render over roughly 0900–1060;
  scrub forward, backward, forward and confirm 0998 renders identically each time
  (`AGENTS.md` §40).
...
- **Card entry frames are not specified by the storyboard.** It gives the motion
  ...
  0976–0998 window in `## Timeline` is this document's proposal, constrained from
  the front by the propagation being visibly under way and from the back by the
  measurement in `## Motion` → **Holds**: the card has to settle at key frame 0998
