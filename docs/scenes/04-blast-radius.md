...
**Immediately before.** Scene 03 (`03-graph-reveal.md`, frames 0510–0870) finished
building the graph and settled: eight nodes and seven edges present, the cascade
laid out in depth, the three crossings drawn at 0734–0809, `withRetry()` accent-lit
at its head, and the camera at rest on the pose it arrived at. There used to be a
scene between the two — `CrossRepoScene`, frames 0870–0960 — and it was cut; see
...
- Storyboard coverage: **`STORYBOARD.md` SCENE 06 — BLAST RADIUS**.
  ...
  whole mapping.
- Global frames: **0870–1010**
- Scene-local frames: **0180–0320**
- Time: **11.5 s – 13.83 s**
...
| Master      | Local       | Beat                                                                                     |
| ----------- | ----------- | ---------------------------------------------------------------------------------------- |
| `0870–0890` | `0000–0020` | `withRetry()` pulses **once**: seven frames up, thirteen down, back to zero.              |
| `0878–0906` | `0008–0036` | Hop 1. The relation lifts at 0188, `Policy.Do()` and `Once()` are marked from 0194.       |
| `0902–0932` | `0032–0062` | Hop 2, the public bridge: `Client.Charge()` and `Client.Refund()`.                        |
| `0928–0966` | `0058–0096` | Hop 3, the three `checkout-service` consumers, four frames apart.                         |
| `0936–0958` | `0066–0088` | The impact card: `opacity 0 → 1`, `x +24 → 0`, no overshoot.                              |
| `0958`      | `0088`      | Card settled, propagation all but complete. **Key visual frame.**                          |
| `0962–1010` | `0092–0140` | The read. 49 frames measured identical; nothing moves, the frame exists to be read.       |
...
At master frame 0870 (local 0180) the frame is **visually identical** to master
frame 0869, inheriting Scene 03's camera and graph exactly:
...
- The settled cascade in depth: `withRetry()` nearest the camera at `z 0.0`,
  ...
  `-Z`. Scene 03's camera move completes at 0838 and nothing in the film moves the
...
At master frame 1010 (local 0320, the boundary frame handed to
`SemanticScene.tsx`):
...
Nothing is mid-animation at 1010, and nothing has been for 49 frames. The next
...
**Hierarchy** at 0958, strongest first:
...
**The pulse.** `withRetry()` pulses exactly **once** at 0870 — a single short
...
key frame 0958 has to hold the count and the evidence in one image. Darkening the
graph there would have taken away the reason to keep the receipt.

**Camera.** Static. Scene 03 leaves the rig at rest at 0870, so this scene holds
...
**Holds.** 0958 must be still enough to export as an image. 0962–1010 is a true
...
It first ran 0950–0990 on a 120-frame cut, and the twenty frames that came off it
...
The card now settles at 0958 and stands 51 frames of this scene, then keeps fading
...
which is what makes the far group legible at 0958.
...
Hard sequence boundary at 0870, and it is **visually continuous rather than
byte-identical**: frames 0869 and 0870 measure **62.93 dB** PSNR, and the
difference is glyph antialiasing and nothing else. Scene 03's camera move
completes at 0838 and nothing in the film moves the camera after it, so the pose
...
At 1010 `SemanticScene.tsx` takes over and the transition comes from the content
...
- the graph must be **at rest** at 1010 — a flatten applied to a still frame reads
...
```text
0870 — single pulse on withRetry(); frame otherwise identical to 0869
0920 — propagation mid-flight, ordering clearly readable
0958 — KEY VISUAL: full blast radius plus impact card, first settled frame
1009 — last frame of the scene; identical to 0962, card and propagation, nothing dimmed
```

**Frame 0958 is a key visual frame** (`STORYBOARD.md` §29, and `AGENTS.md`'s list
of frames that must remain intentional as stills: 0260, 0869, 0958, 1104, 1304,
1470, 1630), earmarked for the README. 0958 is *defined* as the frame the impact
...
- the propagation reads complete and every affected node is unambiguously marked.
  ...
  four frames after the frame stops measurably changing at 0962. The easing is
  front-loaded (`Easing.bezier(0.22, 1, 0.36, 1)`), so at 0958 the last mark is
  already deep in its own tail; if a render ever shows it short, 0962 is its
...
- **One pulse, not a loop.** `withRetry()` pulses exactly once at 0870. No
...
- **Frame 0958 works as a still image.**
- **The card's dwell is not spent.** It settles at 0958, stands 51 frames of this
...
- exact frames of the card's entry, provided it begins no earlier than the
  propagation being visibly under way, is fully settled before 0958, and leaves
...
- exact per-step propagation durations and the small gaps between steps, as long
  as the order stays unambiguous and the propagation completes by 0966, before the
  card is fully read;
...
- The card belongs in `src/components/MetricCard.tsx` (`AGENTS.md` §4, §31) so
  ...
  block at 1010 and fades it out during the flatten, so the two scenes have to
  render it identically; the first version had this scene own the markup, Scene 05
  simply not draw it, and the 1009/1010 seam measured 22 dB PSNR. It is now
  pixel-identical.
...
- Iterate with Remotion Studio plus a small range render over roughly 0860–1020;
  scrub forward, backward, forward and confirm 0958 renders identically each time
  (`AGENTS.md` §40).
...
- **Card entry frames are not specified by the storyboard.** It gives the motion
  ...
  0936–0958 window in `## Timeline` is this document's proposal, constrained from
  the front by the propagation being visibly under way and from the back by the
  measurement in `## Motion` → **Holds**: the card has to settle at key frame 0958
